...
function Onboarding({ onDone }) {
  ...
  const next = () => {
    ...
    if (step === 'pin2') {
      if (pin2 !== pin) { toast(t('pinMismatch'), 'error'); setPin2(''); return; }
      return setStep('role');
    }
  };
        ...
        <motion.div key={step} variants={fadeVariants} initial="hidden" animate="visible" exit="exit"
          ...
          {(step === 'pin' || step === 'pin2') && (
            <>
              ...
              <PinDots value={step === 'pin' ? pin : pin2} onChange={(v) => {
                if (step === 'pin') { setPin(v); if (v.length === 4) setTimeout(() => setStep('pin2'), 150); }
                else { setPin2(v); if (v.length === 4) setTimeout(next, 150); }
              }} />
              <div style={{ flex: 1 }} />
            </>
          )}
