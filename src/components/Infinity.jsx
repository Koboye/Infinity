...
const GroupChatPage = ({ currentUser, users, showToast, onBack }) => {
        ...
        <div style={{ padding: '14px 16px', borderBottom: `1px solid ${COLORS.overlaySubtle}`, display: 'flex', alignItems: 'center', gap: 12, background: COLORS.overlaySubtle }}>
          ...
          <div onClick={()=>setShowGroupInfo(true)} style={{ display:'flex', alignItems:'center', gap:10, flex:1, cursor:'pointer' }}>
            ...
            <div>
              ...
              <div style={{ color: COLORS.surface3, fontSize: 11 }}>{(activeGroup.members || []).length} members · tap for info</div>
            </div>
          </div>
            ...
            <div onClick={e=>e.stopPropagation()} style={{ width:'100%', background:COLORS.surface2, borderTopLeftRadius:28, borderTopRightRadius:28, padding:'20px 20px 40px', maxHeight:'70%', overflowY:'auto' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:20 }}>
                ...
                <div>
                  ...
                  <div style={{ color:COLORS.surface3, fontSize:12 }}>Created by group admin</div>
                </div>
              </div>
              <div style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, marginBottom:12 }}>Members ({groupMembers.length})</div>
              {groupMembers.map(u=>(
                <div key={u.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 0', borderBottom:`1px solid ${COLORS.overlaySubtle}` }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    {u.bio && <div style={{ color:COLORS.surface3, fontSize:11, marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{u.bio}</div>}
        ...
        {groupCallOpen && (
          <div style={{ position:'absolute', inset:0, background:COLORS.bg, zIndex:60, display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'20px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:`1px solid ${COLORS.overlaySubtle}` }}>
              ...
              <button onClick={()=>setGroupCallOpen(null)} style={{ background:'rgba(255,45,85,0.15)', border:'1px solid rgba(255,45,85,0.3)', borderRadius:'50%', width:36, height:36, color:COLORS.brand, cursor:'pointer', fontSize:18 }}>✕</button>
            </div>
            ...
            return (
              <div key={msg.id} style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start', marginBottom: 10 }}>
                ...
                <div style={{ maxWidth: '72%' }}>
                  {!isMine && <div style={{ color: COLORS.surface3, fontSize: 10, marginBottom: 3 }}>@{msg.senderName}</div>}
                  <div style={{ background: isMine ? `linear-gradient(135deg,${COLORS.brand},${COLORS.brandSecondary})` : COLORS.border, borderRadius: isMine ? '18px 18px 4px 18px' : '18px 18px 18px 4px', padding: '10px 14px', color: 'white', fontSize: 14 }}>
                    {msg.text}
  ...
  if (showCreate) return (
    <div style={{ height: '100%', overflow: 'auto', background: COLORS.bg, padding: 16 }}>
      ...
      <div style={{ color: COLORS.surface3, fontSize: 12, marginBottom: 10, fontWeight: 600, textTransform: 'uppercase' }}>Add Members</div>
        ...
        {groups.map(g => (
          <div key={g.id} onClick={() => setActiveGroup(g)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: COLORS.overlaySubtle, borderRadius: 18, marginBottom: 8, cursor: 'pointer', border: `1px solid ${COLORS.overlaySubtle}` }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: COLORS.surface3, fontSize: 12, marginTop: 2 }}>{g.lastMessage || 'No messages yet'} · {(g.members || []).length} members</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding: '14px 16px', borderBottom: `1px solid ${COLORS.overlaySubtle}` }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
          <div style={{ flex: 1, background: COLORS.overlaySubtle, borderRadius: 24, display: 'flex', alignItems: 'center', padding: '10px 14px', gap: 8 }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={COLORS.surface3} strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            ...
            {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', color: COLORS.surface3, cursor: 'pointer', fontSize: 16 }}>✕</button>}
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: COLORS.surface3, cursor: 'pointer', fontSize: 14, padding: 4 }}>Cancel</button>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {filters.map(([id, label]) => (
            <button key={id} onClick={() => setActiveFilter(id)} style={{ background: activeFilter === id ? 'rgba(255,45,85,0.15)' : COLORS.overlaySubtle, border: `1px solid ${activeFilter === id ? 'rgba(255,45,85,0.4)' : COLORS.border}`, borderRadius: 20, padding: '6px 14px', color: activeFilter === id ? COLORS.brand : COLORS.surface3, fontSize: 12, fontWeight: activeFilter === id ? 700 : 400, cursor: 'pointer' }}>{label}</button>
          ))}
        </div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        {!search && (
          <>
            <div style={{ color: COLORS.surface3, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>Trending</div>
            <TrendingHashtags onSearch={t => setSearch(t)} />
            <div style={{ color: COLORS.surface3, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, margin: '20px 0 10px' }}>Suggested Creators</div>
            {users.slice(0, 6).map(u => (
              <div key={u.id} onClick={() => { onViewProfile?.(u.id); onClose(); }} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: `1px solid ${COLORS.overlaySubtle}`, cursor: 'pointer' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: COLORS.surface3, fontSize: 12 }}>{formatNumber(u.followers?.length || 0)} followers</div>
                </div>
              </div>
            ))}
          </>
        ...
        {search && filteredUsers.length > 0 && (
          <>
            <div style={{ color: COLORS.surface3, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>People</div>
            {filteredUsers.map(u => (
              <div key={u.id} onClick={() => { onViewProfile?.(u.id); onClose(); }} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: `1px solid ${COLORS.overlaySubtle}`, cursor: 'pointer' }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: u.avatarColor, display: 'flex', alignItems: 'center', justifyContent: 'center', color: COLORS.textPrimary, fontWeight: 'bold', fontSize: 18, overflow: 'hidden', flexShrink: 0 }}>
                  {u.avatarUrl ? <img src={u.avatarUrl} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" /> : u.avatar}
        ...
        {search && filteredTags.length > 0 && (activeFilter === 'all' || activeFilter === 'hashtag') && (
          <>
            <div style={{ color: COLORS.surface3, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, margin: '16px 0 10px' }}>Hashtags</div>
        ...
        {search && (activeFilter === 'all' || activeFilter === 'video') && filteredVideos.length > 0 && (
          <>
            <div style={{ color: COLORS.surface3, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, margin: '16px 0 10px' }}>Videos</div>
  ...
  return (
    <div style={{ display:'flex', gap:14, padding:'14px 16px', overflowX:'auto', borderBottom:`1px solid ${COLORS.overlaySubtle}`, scrollbarWidth:'none' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, flexShrink:0 }}>
          ...
          <div style={{ width:'100%', height:'100%', borderRadius:'50%',
            ...
            <div style={{ width:'100%', height:'100%', borderRadius:'50%', background: myStories ? COLORS.bg : 'transparent', padding: myStories ? 2 : 0, display:'flex', alignItems:'center', justifyContent:'center' }}>
              <div style={{ width:'100%', height:'100%', borderRadius:'50%', background:currentUser?.avatarColor||COLORS.brand,
                border: !myStories ? `1.5px dashed ${COLORS.surface3}` : 'none',
        ...
        <span style={{ color:COLORS.surface3, fontSize:11 }}>Your story</span>
      </div>
      ...
      {otherStories.map((group, idx) => (
        <div key={group.userId} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:5, flexShrink:0 }}>
          ...
          <span style={{ color:COLORS.surface3, fontSize:11, maxWidth:60, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', textAlign:'center' }}>{group.username.split('_')[0]}</span>
        </div>
      ))}
    </div>
  );
...
const CommentInputBar = ({ currentUser, commentText, setCommentText, onSend, showToast, videoId }) => {
  ...
  return (
    <div style={{padding:'10px 14px',paddingBottom:'max(24px, env(safe-area-inset-bottom))',borderTop:`1px solid ${COLORS.overlaySubtle}`,background:COLORS.bg}}>
      {(previewFile||audioBlob)&&(
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8,background:COLORS.overlaySubtle,borderRadius:14,padding:'8px 12px'}}>
          ...
          <button onClick={clearAttach} style={{marginLeft:'auto',background:'rgba(255,45,85,0.2)',border:'none',borderRadius:'50%',width:22,height:22,color:COLORS.brand,cursor:'pointer',fontSize:13}}>✕</button>
        </div>
      )}
      {showEmoji && (
        <div style={{display:'flex',flexWrap:'wrap',gap:6,padding:'10px 12px',background:COLORS.overlaySubtle,borderRadius:16,marginBottom:8}}>
          {EMOJI_LIST.map(e=>(
...
const PostOptionsMenu = ({ video, currentUser, onClose, showToast, onDelete }) => {
  ...
  const items = [
    ...
    { label:`Block ${name}`, show:!isMine, action:()=>{ showToast?.(`Blocked ${name}`,'info'); }, icon:(<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="4.9" y1="4.9" x2="19.1" y2="19.1"/></svg>) },
    ...
    { label:'Delete Post', show:isMine, danger:true, sep:true, action:()=>{ onDelete?.(video?.id); onClose(); }, icon:(<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>) },
  ].filter(i => i.show);
  return (
    <div style={{ position:'fixed', inset:0, zIndex:5000, background:'rgba(20,15,35,0.45)', display:'flex', alignItems:'flex-end' }} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{ width:'100%', maxHeight:'86vh', overflowY:'auto', background:COLORS.surface, borderTopLeftRadius:26, borderTopRightRadius:26, paddingBottom:'max(20px, env(safe-area-inset-bottom))' }}>
...
const FeedPostCard = ({ video, currentUser, onViewProfile, onOpenComments, onShare, users, onFollow, followed, showToast, onDelete }) => {
  ...
  return (
    ...
    {showOptions && <PostOptionsMenu video={video} currentUser={currentUser} onClose={()=>setShowOptions(false)} showToast={showToast} onDelete={onDelete} />}
  ...
  return (
    <div data-main-scroll="true" onScroll={onFeedScroll} style={{ height:'100%', overflowY:'auto', background:COLORS.bg, padding:'14px 14px max(96px, calc(72px + env(safe-area-inset-bottom)))' }}>
      ...
      <div style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:RADIUS.lg, padding:14, marginBottom:16 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          {[
            {icon:(<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={COLORS.info} strokeWidth="2"><rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="8.5" cy="10" r="1.5"/><path d="M21 15l-5-5L5 19"/></svg>), label:'Photo'},
            {icon:(<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={COLORS.brandSecondary} strokeWidth="2"><rect x="2" y="6" width="14" height="12" rx="2"/><path d="M16 10l6-3v10l-6-3"/></svg>), label:'Video'},
            {icon:(<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={COLORS.brand} strokeWidth="2"><path d="M4 20V10M12 20V4M20 20v-7"/></svg>), label:'Poll'},
            {icon:(<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={COLORS.warning} strokeWidth="2"><circle cx="12" cy="12" r="9"/><path d="M9 10h.01M15 10h.01M8 15s1.5 2 4 2 4-2 4-2"/></svg>), label:'Feeling'},
          ].map(btn=>(
            <button key={btn.label} style={{ display:'flex', alignItems:'center', gap:6, background:COLORS.surfaceAlt, border:'none', borderRadius:14, padding:'7px 12px', color:COLORS.textSecondary, fontSize:12, fontWeight:600, cursor:'pointer' }}>
              {btn.icon}{btn.label}
            </button>
          ))}
        </div>
          ...
          <div style={{ display:'flex', gap:10, overflowX:'auto', paddingBottom:16, marginBottom:6 }}>
            {suggestions.map(u=>(
              <div key={u.id} style={{ flexShrink:0, width:112, background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:16, padding:'12px 10px', textAlign:'center' }}>
                ...
                <button onClick={()=>onFollow?.(u.id)} style={{ width:'100%', background:'none', border:`1px solid ${COLORS.brand}`, color:COLORS.brand, borderRadius:12, padding:'5px 0', fontSize:11.5, fontWeight:700, cursor:'pointer' }}>Follow</button>
              </div>
            ))}
          </div>
...
const CreateScreen = ({ onOpenCamera, onShowSoundLibrary, showToast, t, currentUser, onPosted }) => {
  ...
  const quickActions = [
    ...
    { label:'Poll', color:COLORS.brand, icon:(<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 20V10M12 20V4M20 20v-7"/></svg>), action:()=>showToast?.('Poll builder coming soon','info') },
    { label:'Feeling', color:COLORS.warning, icon:(<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="9"/><path d="M9 10h.01M15 10h.01M8 15s1.5 2 4 2 4-2 4-2"/></svg>), action:()=>showToast?.('Add a feeling','info') },
  ];
  const gridActions = [
    { label:'Tag people', icon:(<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>) },
    { label:'Location', icon:(<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>) },
    ...
    { label:'Activity', icon:(<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>) },
    { label:'Chat', icon:(<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"/></svg>) },
    { label:'GIF', icon:(<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><rect x="3" y="5" width="18" height="14" rx="3"/><path d="M8 10v4M13 10h-2v4h2M16 10v4M16 12h1.5"/></svg>) },
    { label:'Background', icon:(<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 19"/></svg>) },
    { label:'Text', icon:(<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><polyline points="4 7 4 4 20 4 20 7"/><line x1="9" y1="20" x2="15" y2="20"/><line x1="12" y1="4" x2="12" y2="20"/></svg>) },
  ];
  ...
  const submitPost = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'videos'), {
        userId: currentUser?.id, username: currentUser?.username, fullName: currentUser?.fullName||null,
        avatarColor: currentUser?.avatarColor||COLORS.brand, avatarUrl: currentUser?.avatarUrl||null,
        description: text, images, mediaType: images.length ? 'image' : 'text',
        likes:0, comments:0, shares:0, saves:0, views:0, likedBy:[], savedBy:[], createdAt: serverTimestamp(),
      });
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto', background:COLORS.bg }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'16px 16px 14px', background:COLORS.surface, borderBottom:`1px solid ${COLORS.border}` }}>
        ...
        <button onClick={submitPost} disabled={posting || (!text.trim() && !media.length)} style={{ background:COLORS.gradient, opacity:(posting||(!text.trim()&&!media.length))?0.5:1, border:'none', borderRadius:16, padding:'8px 18px', color:'#fff', fontWeight:700, fontSize:13.5, cursor:'pointer' }}>{posting?'Posting…':'Post'}</button>
      </div>
      ...
      <div style={{ padding:16 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:20, flexWrap:'wrap' }}>
          {quickActions.map(btn=>(
            <button key={btn.label} onClick={btn.action} style={{ display:'flex', alignItems:'center', gap:6, background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:14, padding:'8px 13px', color:COLORS.textSecondary, fontSize:12.5, fontWeight:600, cursor:'pointer' }}>
        ...
        <div style={{ background:COLORS.surface2, border:`1px solid ${COLORS.border}`, borderRadius:16, padding:'14px 16px', marginBottom:20 }}>
          ...
          <div style={{ color:COLORS.textTertiary, fontSize:12.5 }}>Let AI suggest a caption based on your photo</div>
        </div>

        {/* Poll / Event / Leaf row */}
        <div style={{ display:'flex', gap:10, marginBottom:22 }}>
          {[
            { label:'Poll', icon:(<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><path d="M4 20V10M12 20V4M20 20v-7"/></svg>) },
            { label:'Event', icon:(<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="3"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>) },
            { label:'Leaf', icon:(<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={COLORS.success} strokeWidth="2"><path d="M11 20A7 7 0 019.8 6.1C15.5 5 17 4.48 19 2c1 2 2 4.18 2 8 0 5.5-4.78 10-10 10z"/><path d="M2 21c0-9 4-13 8-16"/></svg>) },
          ].map(item=>(
            <button key={item.label} onClick={()=>showToast?.(item.label,'info')} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:7, background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:14, padding:'11px 0', color:COLORS.textSecondary, fontSize:12.5, fontWeight:600, cursor:'pointer' }}>
              {item.icon}{item.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding:'16px 16px 0' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
          <div style={{ background:`linear-gradient(135deg,${COLORS.warningAlt},${COLORS.warning})`, borderRadius:22, padding:20 }}>
            ...
            <div style={{ color:COLORS.surface3, fontSize:10, marginTop:2 }}>🪙 Infinity Coins</div>
          </div>
          ...
          <div>
            ...
            {transactions.map(tx=>(
              <div key={tx.id} style={{ background:COLORS.overlaySubtle, borderRadius:16, padding:'13px 14px', marginBottom:8, display:'flex', alignItems:'center', gap:12, border:`1px solid ${COLORS.overlaySubtle}` }}>
                ...
                <div style={{ flex:1 }}><div style={{ color:COLORS.textPrimary, fontSize:12 }}>{tx.label}</div><div style={{ color:COLORS.surface3, fontSize:10, marginTop:2 }}>{tx.date?.toLocaleDateString?.()}</div></div>
        ...
        {(activeTab==='deposit'||activeTab==='withdraw'||activeTab==='convert') && (
          <div style={{ background:COLORS.overlaySubtle, borderRadius:22, padding:20, border:`1px solid ${COLORS.overlaySubtle}` }}>
            <div style={{ color:COLORS.surface3, fontSize:12, marginBottom:8 }}>{activeTab==='deposit'?'Add coins':activeTab==='withdraw'?'Withdraw coins':'Convert to ETH (1 ETH = 10,000 🪙)'}</div>
      ...
      <div style={{ padding:'16px' }}>
        ...
        <div style={{ background:COLORS.surface2, borderRadius:20, overflow:'hidden', marginBottom:20, border:`1px solid ${COLORS.border}` }}>
          <div style={{ padding:'15px 16px', display:'flex', alignItems:'center', gap:14 }}>
            ...
            <div onClick={()=>{ /* theme stored locally */ showToast?.('Theme toggle coming soon','info'); }} style={{ width:46, height:26, background:COLORS.brand, borderRadius:13, position:'relative', cursor:'pointer' }}>
    ...
    <div data-main-scroll="true" onScroll={onFeedScroll} style={{ height:'100%', overflow:'auto', background:COLORS.bg, paddingBottom:'max(96px, calc(72px + env(safe-area-inset-bottom)))' }}>
      <div style={{ position:'relative', paddingBottom:20, background:COLORS.surface, borderRadius:'0 0 24px 24px', boxShadow:'0 2px 14px rgba(124,58,237,0.06)' }}>
        ...
        <div style={{ position:'relative', padding:'14px 16px 0' }}>
          ...
          {user?.link && <a href={user.link} target="_blank" rel="noopener noreferrer" style={{ color:COLORS.info, fontSize:13, display:'block', marginTop:4 }}>{user.link}</a>}
          {user?.location && (
...
const SearchOverlay = ({ onClose, videos, users, onViewProfile }) => {
  ...
  return (
    <div style={{ position:'absolute', inset:0, background:COLORS.bg, zIndex:200, display:'flex', flexDirection:'column' }}>
      {/* ── Header ── */}
      <div style={{ padding:'12px 16px', borderBottom:`1px solid ${COLORS.overlaySubtle}`, display:'flex', gap:10, alignItems:'center', background:'rgba(10,10,10,0.98)', backdropFilter:'blur(20px)' }}>
        <div style={{ flex:1, display:'flex', alignItems:'center', background:COLORS.border, borderRadius:14, padding:'11px 14px', gap:10, border:`1.5px solid ${COLORS.border}`, transition:'border-color 0.2s' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={COLORS.surface3} strokeWidth="2.2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          ...
          {query && <button onClick={()=>setQuery('')} style={{ background:COLORS.border, border:'none', borderRadius:'50%', width:20, height:20, color:COLORS.surface3, cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>✕</button>}
        </div>
        <button onClick={onClose} style={{ background:'none', border:'none', color:COLORS.surface3, fontSize:14, cursor:'pointer', fontWeight:600, padding:'4px 8px' }}>Cancel</button>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto' }}>
          ...
          <div style={{ padding:'0 14px' }}>
            {recentSearches.length > 0 && (
              <div style={{ marginTop:20, marginBottom:24 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                  <span style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1 }}>Recent</span>
                  <button onClick={clearRecentSearches} style={{ background:'none', border:'none', color:COLORS.surface3, fontSize:12, cursor:'pointer' }}>Clear</button>
                </div>
                {recentSearches.map((term,i)=>(
                  <div key={i} onClick={()=>setQuery(term)} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0', borderBottom:`1px solid ${COLORS.overlaySubtle}`, cursor:'pointer' }}>
                    <div style={{ width:34, height:34, borderRadius:'50%', background:COLORS.overlaySubtle, display:'flex', alignItems:'center', justifyContent:'center' }}>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={COLORS.surface3} strokeWidth="2"><polyline points="12 8 12 12 14 14"/><circle cx="12" cy="12" r="10"/></svg>
                    </div>
                    <span style={{ color:COLORS.surface3, fontSize:14, flex:1 }}>{term}</span>
                    <button onClick={e=>{e.stopPropagation(); const u=[...recentSearches]; u.splice(i,1); setRecentSearches(u); try{localStorage.setItem('dagu_recent_searches',JSON.stringify(u));}catch{}}} style={{ background:'none', border:'none', color:COLORS.borderStrong, cursor:'pointer', fontSize:16 }}>✕</button>
                  </div>
                ))}
              </div>
            )}
            <div style={{ marginBottom:8 }}>
              <div style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, marginBottom:14 }}>🔥 Trending</div>
              <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                {trendingSearches.map((tag,i)=>(
                  <button key={i} onClick={()=>{ setQuery(tag); addRecentSearch(tag); }} style={{ background:COLORS.overlaySubtle, border:`1px solid ${COLORS.border}`, borderRadius:20, padding:'8px 14px', color:COLORS.surface3, fontSize:13, cursor:'pointer', fontWeight:600 }}>{tag}</button>
                ))}
              </div>
            </div>
            <div style={{ marginTop:24 }}>
              <div style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, marginBottom:14 }}>Suggested People</div>
              {users.slice(0,5).map(u=>(
                <div key={u.id} onClick={()=>{onViewProfile?.(u.id); addRecentSearch('@'+u.username); onClose();}} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 0', borderBottom:`1px solid ${COLORS.overlaySubtle}`, cursor:'pointer' }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ color:COLORS.textPrimary, fontWeight:700, fontSize:14 }}>@{u.username}</div>
                    <div style={{ color:COLORS.surface3, fontSize:12, marginTop:1 }}>{u.bio?.substring(0,40)||'No bio'}</div>
                  </div>
        ...
        {query.trim() && (
          <div style={{ padding:'8px 14px' }}>
            {totalResults === 0 && (
              <div style={{ textAlign:'center', padding:'60px 20px', color:COLORS.borderStrong }}>
                <div style={{ fontSize:44, marginBottom:12 }}>🔍</div>
            ...
            {(tab==='all'||tab==='users') && results.users.length > 0 && (
              <div style={{ marginBottom:20 }}>
                {tab==='all' && <div style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, marginBottom:12 }}>People</div>}
                {results.users.map(u=>(
                  <div key={u.id} onClick={()=>{onViewProfile?.(u.id); addRecentSearch('@'+u.username); onClose();}} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 12px', background:COLORS.overlaySubtle, borderRadius:16, marginBottom:6, cursor:'pointer', border:`1px solid ${COLORS.overlaySubtle}`, transition:'background 0.1s' }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ color:COLORS.surface3, fontSize:12, marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{u.bio?.substring(0,50)||'No bio'}</div>
                      <div style={{ color:COLORS.borderStrong, fontSize:11, marginTop:1 }}>{(u.followers?.length||0).toLocaleString()} followers</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={COLORS.borderStrong} strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                  </div>
                ))}
              </div>
            )}
            ...
            {(tab==='all'||tab==='videos') && results.videos.length > 0 && (
              <div style={{ marginBottom:20 }}>
                {tab==='all' && <div style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, marginBottom:12 }}>Videos</div>}
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  {results.videos.slice(0,6).map(v=>(
                    <div key={v.id} style={{ aspectRatio:'9/16', position:'relative', borderRadius:14, overflow:'hidden', background:COLORS.surface3, cursor:'pointer' }}>
                      {v.videoUrl?.match(/\.(jpg|jpeg|png|gif|webp)/i) || v.mediaType?.startsWith('image')
                        ? <img src={v.videoUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                        : <video src={v.videoUrl} style={{ width:'100%', height:'100%', objectFit:'cover' }} muted />}
                      <div style={{ position:'absolute', bottom:0, left:0, right:0, background:'linear-gradient(transparent,rgba(0,0,0,0.8))', padding:'20px 8px 8px' }}>
                        ...
                        <div style={{ color:COLORS.surface3, fontSize:10, marginTop:1 }}>{v.likes||0} ❤️</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {(tab==='all'||tab==='hashtags') && results.hashtags.length > 0 && (
              <div style={{ marginBottom:20 }}>
                {tab==='all' && <div style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, marginBottom:12 }}>Hashtags</div>}
                {results.hashtags.map((h,i)=>(
                  <div key={i} onClick={()=>{ setQuery(h); addRecentSearch(h); }} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 12px', background:COLORS.overlaySubtle, borderRadius:14, marginBottom:6, cursor:'pointer', border:`1px solid ${COLORS.overlaySubtle}` }}>
                    ...
                    <div>
                      ...
                      <div style={{ color:COLORS.surface3, fontSize:12 }}>{videos.filter(v=>v.hashtags?.includes(h)).length} posts</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {(tab==='all'||tab==='posts') && results.posts.length > 0 && (
              <div style={{ marginBottom:20 }}>
                {tab==='all' && <div style={{ color:COLORS.surface3, fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:1, marginBottom:12 }}>Posts</div>}
                {results.posts.map(v=>(
                  <div key={v.id} style={{ padding:'12px 14px', background:COLORS.overlaySubtle, borderRadius:14, marginBottom:6, border:`1px solid ${COLORS.overlaySubtle}`, cursor:'pointer' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
                      ...
                      <span style={{ color:COLORS.borderStrong, fontSize:11, marginLeft:'auto' }}>{v.likes||0} ❤️</span>
                    </div>
                    <div style={{ color:COLORS.surface3, fontSize:13, lineHeight:1.5 }}>{v.description?.substring(0,100)}{v.description?.length>100?'...':''}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {filtered.map(sound=>(
          <div key={sound.id} onClick={()=>onSelectSound(sound)} style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 12px', background:COLORS.overlaySubtle, borderRadius:18, marginBottom:8, cursor:'pointer', border:`1px solid ${COLORS.overlaySubtle}` }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ color:COLORS.surface3, fontSize:11, marginTop:2 }}>{sound.artist} · {sound.duration}</div>
            </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12, marginBottom:20 }}>
          {[['Total Views',formatNumber(totalViews),COLORS.brand],['Total Likes',formatNumber(totalLikes),COLORS.brand],['Posts',String(userVideos.length),COLORS.brandSecondary],['Coins',String(user?.coins||0),COLORS.warningAlt]].map(([label,val,color])=>(
            <div key={label} style={{ background:COLORS.overlaySubtle, borderRadius:20, padding:20, border:`1px solid ${COLORS.overlaySubtle}` }}>
              <div style={{ color:COLORS.surface3, fontSize:11, textTransform:'uppercase', letterSpacing:0.5 }}>{label}</div>
          ...
          <div style={{ height:120, display:'flex', alignItems:'flex-end', gap:6 }}>
            {weeklyData.map((v,i)=>(
              <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                ...
                <span style={{ color:COLORS.surface3, fontSize:9 }}>{['M','T','W','T','F','S','S'][i]}</span>
              </div>
            ))}
          </div>
        </div>
...
/* Bottom nav: each tab keeps its own signature color (matches reference mockup) — not a single active/inactive tint */
const NAV_TAB_COLORS = { home:'#3B82F6', friends:'#14B8A6', inbox:'#EC4899', profile:'#F59E0B' };
const TabIcon = ({id, active, currentUser}) => {
  const color = NAV_TAB_COLORS[id] || COLORS.brand;
  ...
  if(id==='home') return (
    <div style={{ position:'relative' }}>
      <div style={{ width:38, height:38, borderRadius:'50%', background:active?`${color}1F`:'transparent', display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      {active && <div style={{ position:'absolute', bottom:-6, left:'50%', transform:'translateX(-50%)', width:4, height:4, borderRadius:'50%', background:color }} />}
    </div>
  );
  ...
  if(id==='profile') return (
    <div style={{ position:'relative' }}>
      <div style={{ width:30, height:30, borderRadius:'50%', border:`2px solid ${color}`, padding:1.5, display:'flex', alignItems:'center', justifyContent:'center', opacity: active?1:0.85 }}>
        <div style={{ width:'100%', height:'100%', borderRadius:'50%', background:currentUser?.avatarColor||color, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:700, fontSize:11, overflow:'hidden' }}>
          {currentUser?.avatarUrl ? <img src={currentUser.avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" /> : (currentUser?.username||'?')[0]?.toUpperCase()}
        </div>
      </div>
      {active && <div style={{ position:'absolute', bottom:-6, left:'50%', transform:'translateX(-50%)', width:4, height:4, borderRadius:'50%', background:color }} />}
