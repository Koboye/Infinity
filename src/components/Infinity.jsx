...
// Note: getDisplayHandle (format.js) and useLiveTranslation (translate.js) are exported
// but currently unused anywhere in the app — pre-existing dead code found during the
// Phase 2 extraction, not removed here since that's a behavior change outside this pass' scope.
    ...
    <AnimatePresence>
      {req && (
        <motion.div
          ...
          style={{ position:'fixed', inset:0, zIndex:Z.confirmDialog, background:'rgba(11,15,25,0.5)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
        >
          <motion.div
...
/* SHADOW / EASE / TRANSITION now live in @/lib/theme (single source — see
   theme.js for the drift this fixed). The old `pressable` export was dead
   code (zero call sites) and was dropped rather than moved. */
...
const OfflineBanner = () => (
  <div style={{ position:'fixed', top:0, left:0, right:0, zIndex:Z.systemBanner, background:'#FFB100', padding:'10px 16px', display:'flex', alignItems:'center', gap:8, justifyContent:'center', animation:'slideDown 0.3s ease' }}>
...
const LOGIN_METHODS = [
  ...
  { id: 'email', name: 'Email', icon: '📧', color: COLORS.brand },
];
...
const SheetBackHeader = ({ title, onClose, right }) => (
  <div style={{ display:'flex', alignItems:'center', gap:14, padding:'16px 16px 14px', borderBottom:`1px solid ${COLORS.border}` }}>
    <button onClick={onClose} aria-label="Close" style={{ background:'none', border:'none', cursor:'pointer', color:COLORS.textPrimary, padding:4, display:'flex' }}>
...
const ReportReasonSheet = ({ title = 'Report', onClose, onSubmit }) => {
  ...
  return (
    <motion.div
      variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
      style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'rgba(20,15,35,0.55)', display:'flex', alignItems:'flex-end' }} onClick={e=>{ e.stopPropagation(); onClose(); }}>
      ...
      <motion.div
        ...
        onClick={e=>e.stopPropagation()} style={{ width:'100%', maxHeight:'70vh', display:'flex', flexDirection:'column', background:COLORS.surface, borderTopLeftRadius:22, borderTopRightRadius:22, paddingBottom:'max(14px, env(safe-area-inset-bottom))' }}>
...
const ShareSheet = ({ video, currentUser, onClose, showToast }) => {
  ...
  return (
    <motion.div
      ...
      <motion.div
        variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
        onClick={e=>e.stopPropagation()} style={{ width:'100%', maxHeight:'70vh', overflowY:'auto', background:COLORS.surface, borderTopLeftRadius:20, borderTopRightRadius:20, paddingBottom:'max(16px, env(safe-area-inset-bottom))' }}>
    ...
    <div style={{ background: '#1C1C24', borderRadius: 20, padding: 14, border: '1px solid rgba(255,255,255,0.08)' }}>
      <div style={{ display: 'flex', gap: 8, marginBottom: 10, overflowX: 'auto' }}>
        {STICKER_PACKS.map((pack, i) => (
          <button key={pack.id} onClick={() => setActivePack(i)} style={{ background: i === activePack ? 'rgba(11,95,255,0.2)' : 'transparent', border: i === activePack ? '1px solid rgba(11,95,255,0.4)' : '1px solid transparent', borderRadius: 20, padding: '4px 12px', color: i === activePack ? COLORS.brand : 'rgba(255,255,255,0.4)', fontSize: 12, cursor: 'pointer', flexShrink: 0 }}>
            {pack.name}
          </button>
        ))}
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {STICKER_PACKS[activePack].stickers.map(s => (
          <button key={s} onClick={() => onSelect(s)} aria-label="Send sticker" style={{ background: 'none', border: 'none', fontSize: 28, cursor: 'pointer', padding: 4, borderRadius: 8, transition: 'transform 0.1s' }}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: COLORS.bg, zIndex: 60, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '20px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${COLORS.overlaySubtle}` }}>
        ...
        <button onClick={onClose} aria-label="Close" style={{ background: 'rgba(11,95,255,0.15)', border: '1px solid rgba(11,95,255,0.3)', borderRadius: '50%', width: 36, height: 36, color: COLORS.brand, cursor: 'pointer', fontSize: 18 }}>✕</button>
      </div>
      ...
      <div style={{ padding: '16px 20px 32px', display: 'flex', justifyContent: 'center', gap: 20, borderTop: `1px solid ${COLORS.overlaySubtle}` }}>
        ...
        <button onClick={onClose} aria-label="End call" style={{ width: 56, height: 56, borderRadius: '50%', background: COLORS.danger, border: 'none', color: 'white', fontSize: 22, cursor: 'pointer' }}>📵</button>
      </div>
    </div>
  );
};
          ...
          <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,0.85)', zIndex:50, display:'flex', alignItems:'flex-end' }} onClick={()=>setShowGroupInfo(false)}>
            <div onClick={e=>e.stopPropagation()} style={{ width:'100%', background:COLORS.surface2, borderTopLeftRadius:28, borderTopRightRadius:28, padding:'20px 20px 40px', maxHeight:'70%', overflowY:'auto' }}>
                ...
                <div key={u.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 0', borderBottom:`1px solid ${COLORS.overlaySubtle}` }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ color:COLORS.textPrimary, fontSize:14, fontWeight:700, display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                      @{u.username}
                      {u.id===currentUser?.id && <span style={{ color:COLORS.successText, fontSize:10 }}>You</span>}
              ...
              <div
                ...
                <span style={{ color:COLORS.warningText, fontSize:14, fontWeight:600 }}>Leave Group</span>
              </div>
              ...
              {currentUser?.id===activeGroup?.admin && (
                <div
                  ...
                  <span style={{ color:COLORS.dangerText, fontSize:14, fontWeight:600 }}>Delete Group</span>
                </div>
              )}
            </div>
          </div>
    ...
    <div style={{ height: '100%', overflow: 'auto', background: COLORS.bg }}>
      <div style={{ padding: '16px 16px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${COLORS.overlaySubtle}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={onBack} aria-label="Back" style={{ background: 'none', border: 'none', color: COLORS.textPrimary, cursor: 'pointer', fontSize: 18 }}>←</button>
...
const SavedPostsPage = ({ currentUser, onClose, showToast }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: COLORS.bg, zIndex:Z.page, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${COLORS.overlaySubtle}` }}>
        ...
        <button onClick={onClose} aria-label="Close" style={{ background: COLORS.border, border: 'none', borderRadius: '50%', width: 32, height: 32, color: COLORS.textPrimary, cursor: 'pointer', fontSize: 16 }}>✕</button>
      </div>
...
const BroadcastPage = ({ currentUser, users, showToast, onClose }) => {
  ...
  const [bgColor, setBgColor] = useState(COLORS.brand);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex:Z.page, background: bgColor, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '14px 16px', borderBottom: `1px solid ${COLORS.overlaySubtle}` }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
          <div style={{ flex: 1, background: COLORS.overlaySubtle, borderRadius: 24, display: 'flex', alignItems: 'center', padding: '10px 14px', gap: 8 }}>
            ...
            {search && <button onClick={() => setSearch('')} aria-label="Clear search" style={{ background: 'none', border: 'none', color: COLORS.textTertiary, cursor: 'pointer', fontSize: 16 }}>✕</button>}
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        ...
        {search && (activeFilter === 'all' || activeFilter === 'video') && filteredVideos.length > 0 && (
          <>
            ...
            <motion.div variants={listContainerVariants} initial="hidden" animate="visible" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {filteredVideos.slice(0, 8).map(v => (
                <motion.div key={v.id} variants={listItemVariants} style={{ aspectRatio: '9/16', position: 'relative', borderRadius: 14, overflow: 'hidden', background: COLORS.surface4 }}>
                  {v.videoUrl?.match(/\.(jpg|jpeg|png|gif|webp)/i) || v.mediaType?.startsWith('image')
...
const GlobalStyles = () => (
  <style>{`
    ...
    ──────────────────────────────────────────────────────────────────── */
    /* A :root{} block of CSS custom properties (--accent, --success, --teal-2,
       --verified, etc.) used to live here. Removed — none of them were ever
       read via var(...) anywhere in the app (confirmed: only --x/--y, set
       per-element for the ripple effect, and --app-bg, set by theme.js's
       toggleTheme, are real). It was dead scaffolding from an earlier
       CSS-variable approach, abandoned in favor of the COLORS object above.
       Notably --teal-2 and --verified shared the identical value #3B82F6
       despite --teal-2 not being a teal at all — a symptom of it being
       unused rather than a naming bug worth preserving. */
    ...
    *{scrollbar-width:none;-ms-overflow-style:none}
    ...
    /* toastIn/toastOut keyframes removed — Toast now animates via framer-motion (see src/lib/motion.js toastVariants) */
    ...
    .story-avatar-ring{background:conic-gradient(${COLORS.brand},${COLORS.brandLight},${COLORS.brand});padding:2.5px;border-radius:50%}
  ...
  return (
    <div style={{ position:'relative', overflow:'hidden', ...style }}>
      ...
      {error
        ...
        : <img loading="lazy" decoding="async" src={src} alt={alt||''} onLoad={()=>setLoaded(true)} onError={()=>setError(true)} style={{ width:'100%', height:'100%', objectFit:'cover', opacity: loaded?1:0, transition:'opacity 0.3s ease' }} />
      }
    </div>
  );
};
...
const NotifPopup = ({ notif, user, onClose, onTap }) => {
  ...
  const icons = { like:'❤️', comment:'💬', follow:'👤', mention:'@', gift:'🎁', live:'🔴', call:'📞', message:'✉️' };
  ...
  return (
    <div
      ...
      style={{ position:'fixed', top:52, left:12, right:12, zIndex:Z.toast,
      ...
      <div style={{ position:'relative', flexShrink:0 }}>
        <div style={{ width:44, height:44, borderRadius:'50%', background:user?.avatarColor||COLORS.brand, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:'bold', fontSize:18, overflow:'hidden' }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ color:'white', fontSize:13, fontWeight:600, lineHeight:1.35, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
          <span style={{ color:COLORS.brand }}>@{user?.username||'someone'}</span>{' '}{notif?.message}
        </div>
        <div style={{ color:'rgba(255,255,255,0.35)', fontSize:11, marginTop:2 }}>Just now · Swipe to dismiss</div>
      </div>
      <button onClick={e=>{e.stopPropagation();onClose();}} aria-label="Close" style={{ background:'rgba(255,255,255,0.08)', border:'none', borderRadius:'50%', width:26, height:26, color:'rgba(255,255,255,0.5)', cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>✕</button>
      <div style={{ position:'absolute', bottom:0, left:0, right:0, height:2, background:'rgba(255,255,255,0.06)', borderRadius:'0 0 20px 20px', overflow:'hidden' }}>
        <div style={{ height:'100%', background:`linear-gradient(90deg,${COLORS.brandLight},${COLORS.brand})`, animation:'notifBar 4.5s linear forwards' }}/>
      </div>
    </div>
  );
};
...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <motion.div
      ...
      style={{ position:'fixed', bottom:110, left:'50%', zIndex:Z.toast, display:'flex', alignItems:'center', gap:10, background:'rgba(15,15,15,0.95)', backdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:40, padding:'10px 18px 10px 10px', boxShadow:SHADOW.modal, whiteSpace:'nowrap' }}>
...
const TelegramStoryViewer = ({ storyGroups, startGroupIdx, currentUser, onClose, onViewProfile, showToast }) => {
  ...
  const sendReply = async () => {
    ...
    // AI content check — story replies route into the same DM thread as regular
    // messages, so they carry the same abuse risk and get the same check. Fail
    // open on a moderation-service hiccup so a network blip never blocks a
    // legitimate reply.
    try {
      const modResult = await apiFetch('/api/moderation/check', { method: 'POST', body: JSON.stringify({ text: replyText.trim() }) });
      if (modResult?.blocked) {
        showToast?.('This reply looks like it may violate our community guidelines and was not sent.', 'error');
        return;
      }
    } catch (e) {}
    try {
  ...
  // Root stays an instant appear rather than backdropVariants/pageVariants — a
  // story is tapped from a live, already-progressing ring on the Stories row,
  // and its own 5s-per-story timer starts the moment it mounts. An eased-in
  // entrance would visibly eat into that timer before the story is even fully
  // on screen. Same reasoning applies to CreateStoryModal's capture screen.
  return (
    <div style={{ position:'fixed', inset:0, background:'#000', zIndex:Z.page, display:'flex', flexDirection:'column', userSelect:'none' }}>
      {/* Progress bars */}
      ...
      <div style={{ position:'absolute', top:20, left:0, right:0, zIndex:20, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 14px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }}
          ...
          <div style={{ width:42, height:42, borderRadius:'50%', background:currentGroup.avatarColor||COLORS.brand,
            ...
            {currentGroup.avatarUrl
              ? <img loading="lazy" decoding="async" src={currentGroup.avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" />
            ...
            {(currentStory.seenBy?.length || 0) > 0 && (
              <div style={{ display:'flex', gap:-8 }}>
                {(currentStory.seenBy || []).slice(0,6).map((uid,i)=>{
                  ...
                  return (
                    <div key={uid} style={{ width:28, height:28, borderRadius:'50%', background:COLORS.brandTextFill, border:'2px solid #000', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:11, fontWeight:'bold', marginLeft: i>0?-8:0 }}>
                      {uid[0]?.toUpperCase()}
                    </div>
                  );
                })}
      ...
      {/* Replay overlay — shown instead of auto-closing when the last story ends.
          Unlike the viewer's own instant appear (see notes below), this is a
          plain confirmation surface layered on top of an already-open viewer,
          not a camera/media capture moment — fadeVariants fits it the same way
          it fits a dropdown or popover. */}
      <AnimatePresence>{finished && (
        <motion.div
          variants={fadeVariants} initial="hidden" animate="visible" exit="exit"
          style={{ position:'fixed', inset:0, zIndex:40, background:'rgba(0,0,0,0.85)', backdropFilter:'blur(6px)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:18 }}>
          <div style={{ width:76, height:76, borderRadius:'50%', background:currentGroup.avatarColor||COLORS.brand, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:'bold', fontSize:28, overflow:'hidden', border:'3px solid white' }}>
            {currentGroup.avatarUrl ? <img loading="lazy" decoding="async" src={currentGroup.avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" /> : (currentGroup.username||'?')[0].toUpperCase()}
          </div>
          ...
          <div style={{ display:'flex', gap:12 }}>
            ...
            <button onClick={onClose} style={{ background:COLORS.gradient, border:'none', borderRadius:24, padding:'12px 26px', color:'white', fontWeight:700, fontSize:14, cursor:'pointer' }}>Close</button>
          </div>
        </motion.div>
      )}</AnimatePresence>
    </div>
        ...
        <>
          <div style={{ position:'absolute', top:8, left:8, width:30, height:30, borderRadius:'50%', background: isLive ? `linear-gradient(135deg,${COLORS.live},#B3121F)` : `linear-gradient(135deg,${COLORS.brand},${COLORS.brandSecondary})`, padding:2.5, boxSizing:'border-box' }}>
            <div style={{ width:'100%', height:'100%', borderRadius:'50%', background: avatarColor || COLORS.brand, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:700, fontSize:12, overflow:'hidden' }}>
              {avatarUrl ? <img loading="lazy" decoding="async" src={avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" /> : initial}
            </div>
          </div>
...
const StoriesPage = ({ users, currentUser, onClose, onViewStory, onCreateStory, showToast, t }) => {
  ...
  // Same savedBy/arrayUnion pattern FeedPostCard.toggleSave uses for videos
  // (see SavedPostsPage) — mirrors an existing, working pattern instead of
  // inventing a new persistence shape for stories specifically.
  const toggleFavoriteStory = async (storyId, currentlySaved) => {
    if (!storyId || !currentUser?.id) return;
    try {
      await updateDoc(doc(db, 'stories', storyId), {
        savedBy: currentlySaved ? arrayRemove(currentUser.id) : arrayUnion(currentUser.id),
      });
    } catch (e) { showToast?.('Could not update favorite', 'error'); }
  };

  // Was purely cosmetic before — cleared the input and showed "Reaction sent"
  // with nothing actually written anywhere, so the story owner never saw it.
  // Routes through the same 1:1 conversation thread TelegramStoryViewer.sendReply
  // and ConversationView use, so it shows up as a real chat message.
  const sendStoryReaction = async () => {
    const text = msg.trim();
    const otherUserId = active?.userId;
    const storyId = activeStory?.id;
    if (!text || !otherUserId || !storyId || otherUserId === currentUser?.id) { setMsg(''); return; }
    try {
      const convId = [currentUser.id, otherUserId].sort().join('_');
      await addDoc(collection(db, 'messages', convId, 'msgs'), {
        from: currentUser.id, to: otherUserId,
        text: `↩️ Replied to your story: ${text}`,
        createdAt: serverTimestamp(), status: 'sent', expireAt: null,
      });
      await setDoc(doc(db, 'conversations', convId), {
        participants: [currentUser.id, otherUserId],
        lastMessage: `↩️ Replied to your story: ${text}`,
        lastMessageAt: serverTimestamp(),
        lastSenderId: currentUser.id,
        lastMessageStatus: 'sent',
        hiddenFor: [],
      ...
      await updateDoc(doc(db, 'stories', storyId), { replyCount: increment(1) }).catch(() => {});
      showToast?.('Reply sent ✓', 'success');
  ...
  const others = storyUsers.filter(g => g.userId !== currentUser?.id && (!q || g.username?.toLowerCase().includes(q)));
  ...
  // Filtering can shrink the list out from under whatever index was active
  // (e.g. typing a search while on tile 4 of a now-2-tile list) — clamp so we
  // never render tiles[activeIdx] as undefined.
  useEffect(() => { if (activeIdx >= tiles.length) setActiveIdx(0); }, [tiles.length, activeIdx]);
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:Z.page, background:COLORS.bg, display:'flex', flexDirection:'column' }}>
      {/* Header */}
      <div style={{ padding:'16px 16px 0' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          ...
          <button onClick={()=>setShowFavorites(true)} aria-label="Favorite stories" style={{ background:'none', border:'none', cursor:'pointer', color:COLORS.textSecondary }}>
      ...
      <div style={{ display:'flex', gap:12, padding:'0 16px 18px', overflowX:'auto' }}>
        {tiles.map((tile,i)=>(
          <div key={tile.userId||i} onClick={()=>{ if(tile.mine && !tile.stories.length){ onCreateStory?.(); } else { setActiveIdx(i); } }} style={{ position:'relative', flexShrink:0, width:96, height:130, borderRadius:18, overflow:'hidden', cursor:'pointer', border:activeIdx===i?`2.5px solid ${COLORS.brand}`:`1px solid ${COLORS.border}`, background:COLORS.surfaceAlt }}>
            {tile.stories?.[0]?.mediaUrl ? (
              <img loading="lazy" decoding="async" src={tile.stories[0].mediaUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
            ) : (
              <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', background:tile.avatarColor||COLORS.brand, color:'#fff', fontWeight:700, fontSize:26 }}>
                {tile.avatarUrl ? <img loading="lazy" decoding="async" src={tile.avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" /> : (tile.username||'?')[0]?.toUpperCase()}
              </div>
            )}
            ...
            {tile.stories?.[0]?.id && (
              <button
                onClick={e=>{ e.stopPropagation(); toggleFavoriteStory(tile.stories[0].id, tile.stories[0].savedBy?.includes(currentUser?.id)); }}
                aria-label={tile.stories[0].savedBy?.includes(currentUser?.id) ? 'Remove from favorites' : 'Add to favorites'}
                style={{ position:'absolute', top:8, right:8, width:24, height:24, borderRadius:'50%', background:'rgba(0,0,0,0.35)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', padding:0 }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill={tile.stories[0].savedBy?.includes(currentUser?.id) ? '#FF3B30' : 'none'} stroke={tile.stories[0].savedBy?.includes(currentUser?.id) ? '#FF3B30' : '#fff'} strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>
              </button>
            )}
        ...
        {q && !others.length && (
          <div style={{ flexShrink:0, display:'flex', alignItems:'center', color:COLORS.textTertiary, fontSize:12.5, padding:'0 8px' }}>
            No stories match "{storySearch.trim()}"
          </div>
        )}
      </div>
      ...
      <div style={{ flex:1, margin:'0 16px 16px', borderRadius:22, overflow:'hidden', position:'relative', background: activeStory?.bgColor || '#1a1a24', minHeight:0 }}>
        {isActiveImage ? (
          <img loading="lazy" decoding="async" src={activeMedia} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', position:'absolute', inset:0 }} />
        ) : isActiveVideo ? (
          ...
          <>
            <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', background:active?.avatarColor||COLORS.brand, overflow:'hidden' }}>
              {active?.avatarUrl ? <img loading="lazy" decoding="async" src={active.avatarUrl} style={{ width:'100%', height:'100%', objectFit:'cover' }} alt="" /> : <span style={{ color:'#fff', fontWeight:800, fontSize:64 }}>{(active?.username||'?')[0]?.toUpperCase()}</span>}
            </div>
      ...
      {showFavorites && (
        <div style={{ position:'absolute', inset:0, background:COLORS.bg, zIndex:Z.modal, display:'flex', flexDirection:'column' }}>
          <div style={{ padding:'16px', display:'flex', alignItems:'center', gap:12, borderBottom:`1px solid ${COLORS.overlaySubtle}` }}>
            <button onClick={()=>setShowFavorites(false)} aria-label="Back" style={{ background:'none', border:'none', color:COLORS.textPrimary, cursor:'pointer', fontSize:18 }}>←</button>
            <div style={{ color:COLORS.textPrimary, fontWeight:800, fontSize:18 }}>Favorite Stories</div>
          </div>
          <div style={{ flex:1, overflowY:'auto', padding:16 }}>
            {(() => {
              const favTiles = tiles.filter(tl => tl.stories?.[0]?.savedBy?.includes(currentUser?.id));
              if (!favTiles.length) return (
                <div style={{ textAlign:'center', padding:60, color:COLORS.textTertiary }}>
                  <div style={{ fontSize:44, marginBottom:12 }}>♡</div>
                  <div style={{ fontSize:14 }}>No favorite stories yet</div>
                  <div style={{ fontSize:12, marginTop:6 }}>Tap the heart on any story to save it here</div>
                </div>
              );
              return (
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                  {favTiles.map(tl => (
                    <div key={tl.userId} onClick={()=>{ setActiveIdx(tiles.findIndex(x=>x.userId===tl.userId)); setShowFavorites(false); }} style={{ aspectRatio:'9/16', borderRadius:12, overflow:'hidden', position:'relative', cursor:'pointer', background:COLORS.surfaceAlt }}>
                      {tl.stories[0]?.mediaUrl
                        ? <img loading="lazy" decoding="async" src={tl.stories[0].mediaUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                        : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', background:tl.avatarColor||COLORS.brand, color:'#fff', fontWeight:700, fontSize:22 }}>{(tl.username||'?')[0]?.toUpperCase()}</div>}
                      <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'16px 6px 6px', background:'linear-gradient(180deg,transparent,rgba(0,0,0,0.6))', color:'#fff', fontSize:10.5, fontWeight:700, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{tl.mine ? 'Your story' : tl.username}</div>
                    </div>
                  ))}
                </div>
              );
...
const CreateStoryModal = ({ currentUser, onClose, showToast }) => {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'stories'), {
        ...
        avatarColor: currentUser.avatarColor || COLORS.brand,
  ...
  if (!mode) return (
    <motion.div
      ...
      <motion.div
        variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
        onClick={e=>e.stopPropagation()} style={{ width:'100%', background:'#15151C', borderTopLeftRadius:32, borderTopRightRadius:32, padding:'20px 20px 44px', border:'1px solid rgba(255,255,255,0.06)' }}>
  ...
  // Camera/text/audio/gallery capture screen: intentionally left as an instant
  // appear rather than sheetVariants/backdropVariants. This is the same call
  // made for TelegramStoryViewer below — once the person has picked a capture
  // mode, an eased-in transition sits between the tap and the camera actually
  // being ready (getUserMedia has its own latency), which reads as the UI being
  // slow rather than deliberate. The picker sheet above it still animates since
  // that's a plain choice sheet, not a capture surface.
  return (
    <div style={{ position:'fixed', inset:0, background:'#000', zIndex:Z.modal, display:'flex', flexDirection:'column' }}>
      <div style={{ padding:'16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <button onClick={handlePost} disabled={uploading} style={{ background:COLORS.gradient, border:'none', borderRadius:20, padding:'8px 16px', color:'white', fontWeight:700, cursor:'pointer', fontSize:13, opacity:uploading?0.6:1 }}>{uploading?'Posting...':'Post'}</button>
      </div>
      <div style={{ flex:1, position:'relative', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
        {mode==='camera' && (
          <div style={{ width:'100%', height:'100%', position:'relative' }}>
            <video ref={videoRef} autoPlay playsInline style={{ width:'100%', height:'100%', objectFit:'cover' }} />
            <button onClick={capturePhoto} aria-label="Take photo" style={{ position:'absolute', bottom:30, left:'50%', transform:'translateX(-50%)', background:'white', border:'4px solid rgba(255,255,255,0.4)', borderRadius:'50%', width:72, height:72, cursor:'pointer', fontSize:28 }}>📸</button>
          </div>
        )}
        {mode==='text' && (
        ...
        {mode==='file' && selectedFile && (
          selectedFile.type.startsWith('video/') ? <video src={selectedFile.url} style={{ width:'100%', height:'100%', objectFit:'cover' }} controls /> : <img loading="lazy" decoding="async" src={selectedFile.url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
        )}
      </div>
    </div>
  );
...
const DonationSheet = ({ recipient, currentUser, onClose, showToast, currency = 'NGN', onSuccess }) => {
  ...
  return (
    <motion.div
      variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
      onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex:Z.modal, background: 'rgba(15,10,25,0.55)', display: 'flex', alignItems: 'flex-end', maxWidth: 430, margin: '0 auto' }}>
      <motion.div
        variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
        onClick={e => e.stopPropagation()} style={{ width: '100%', background: '#15151C', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: '20px 20px max(24px, env(safe-area-inset-bottom))' }}>
        ...
        {status === 'success' ? (
          ...
          <>
            ...
            <button
              ...
              style={{
                width: '100%', background: `linear-gradient(135deg,${COLORS.brand},#6E4CF5)`, border: 'none', borderRadius: 16,
  ...
  return (
    ...
    >
      ...
      <div style={{ position:'relative', width:'100%', height: dragging ? 4 : 2.5, background:'rgba(255,255,255,0.25)', borderRadius:3, transition:'height 0.15s ease' }}>
        <div style={{ position:'absolute', left:0, top:0, height:'100%', borderRadius:3, background:`linear-gradient(90deg,${COLORS.brandLight},${COLORS.brand})`, width:`${progress}%`, transition: dragging ? 'none' : 'width 0.5s linear' }} />
        <div style={{
...
const MediaCarousel = ({ images, onDoubleClick, maxHeight = 420 }) => {
  ...
  if (images.length === 1) {
    return <img loading="lazy" decoding="async" src={images[0]} alt="" onDoubleClick={onDoubleClick} style={{ width:'100%', maxHeight, display:'block', objectFit:'cover' }} />;
  }
  ...
  return (
    <div style={{ position:'relative' }}>
      ...
      >
        {images.map((src, i) => (
          <img loading="lazy" decoding="async" key={i} src={src} alt="" draggable={false} style={{ flex:'0 0 100%', width:'100%', maxHeight, objectFit:'cover', scrollSnapAlign:'center', display:'block' }} />
        ))}
      </div>
...
const LikesModal = ({ video, currentUser, users, onClose, onFollow, followed }) => {
  ...
  return (
    <motion.div
      ...
      style={{ position:'fixed', inset:0, zIndex:Z.modal, background:COLORS.surface, display:'flex', flexDirection:'column' }}>
...
const CommentsModal = ({ video, currentUser, onClose, showToast, onViewProfile }) => {
  ...
  return (
    <motion.div
      variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
      onClick={onClose} style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'rgba(15,10,25,0.5)', display:'flex', alignItems:'flex-end', maxWidth:430, margin:'0 auto' }}>
      <motion.div
        variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
        onClick={e=>e.stopPropagation()} style={{ width:'100%', height:'58vh', maxHeight:520, background:COLORS.surface, borderTopLeftRadius:24, borderTopRightRadius:24, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:14, padding:'10px 16px 14px', borderBottom:`1px solid ${COLORS.border}` }}>
        ...
        <button onClick={onClose} aria-label="Close" style={{ position:'absolute', right:16, background:'none', border:'none', cursor:'pointer', color:COLORS.textTertiary, padding:4, display:'flex' }}>
        ...
        {comments.map(c => {
          ...
          <div key={c.id} style={{ display:'flex', gap:10, marginBottom:18 }}>
            <div onClick={()=>onViewProfile?.(c.userId)} style={{ width:38, height:38, borderRadius:'50%', background:c.avatarColor||COLORS.brand, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:700, fontSize:13, overflow:'hidden', flexShrink:0, cursor:'pointer' }}>
              {c.avatarUrl ? <img loading="lazy" decoding="async" src={c.avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" /> : (c.avatar||'U')}
            </div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', alignItems:'baseline', gap:8 }}>
                ...
                <div style={{ position:'relative' }}>
                  <button onClick={()=>setOpenMenuFor(m=>m===c.id?null:c.id)} aria-label="More options" style={{ background:'none', border:'none', cursor:'pointer', color:COLORS.textTertiary, padding:'2px 4px', display:'flex' }}>
                  ...
                  {openMenuFor === c.id && (
                    <div style={{ position:'absolute', top:'calc(100% + 4px)', right:0, background:COLORS.surface3, border:`1px solid ${COLORS.border}`, borderRadius:12, padding:4, zIndex:10, minWidth:120, boxShadow:'0 8px 24px rgba(0,0,0,0.3)' }}>
                      ...
                      {canModerate && (
                        <div onClick={()=>deleteComment(c)} style={{ padding:'8px 10px', borderRadius:8, color:COLORS.dangerText, fontSize:12.5, cursor:'pointer' }}>Delete</div>
                      )}
                    </div>
                  )}
                </div>
      ...
      {cmAttachment && (
        <div style={{ padding:'0 16px 6px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, background:COLORS.overlaySubtle, borderRadius:14, padding:'8px 12px' }}>
            {cmAttachment.type?.startsWith('image') && <img loading="lazy" decoding="async" src={cmAttachment.url} alt="" style={{ height:44, width:44, objectFit:'cover', borderRadius:8 }}/>}
            {cmAttachment.type?.startsWith('video') && <video src={cmAttachment.url} style={{ height:44, width:60, objectFit:'cover', borderRadius:8 }}/>}
            <button onClick={()=>setCmAttachment(null)} aria-label="Remove attachment" style={{ marginLeft:'auto', background:'rgba(11,95,255,0.2)', border:'none', borderRadius:'50%', width:22, height:22, color:COLORS.brand, cursor:'pointer', fontSize:13 }}>✕</button>
          </div>
        </div>
      )}
...
const SaveConfirmSheet = ({ onClose, onViewCollections }) => (
  <motion.div
    variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
    style={{ position:'fixed', inset:0, zIndex:Z.drawer, background:'rgba(20,15,35,0.45)', display:'flex', alignItems:'flex-end' }} onClick={onClose}>
    <motion.div
      variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
      onClick={e=>e.stopPropagation()} style={{ width:'100%', background:COLORS.surface, borderTopLeftRadius:26, borderTopRightRadius:26, paddingBottom:'max(24px, env(safe-area-inset-bottom))' }}>
...
const PostOptionsMenu = ({ video, currentUser, onClose, showToast, onDelete, onBlock }) => {
  ...
  return (
    <motion.div
      ...
      <motion.div
        variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
        onClick={e=>e.stopPropagation()} style={{ width:'100%', maxHeight:'70vh', overflowY:'auto', background:COLORS.surface, borderTopLeftRadius:22, borderTopRightRadius:22, paddingBottom:'max(14px, env(safe-area-inset-bottom))' }}>
...
const FeedPostCard = ({ video, currentUser, onViewProfile, onOpenComments, onShare, users, onFollow, followed, showToast, onDelete, onBlock, isLive }) => {
  ...
  return (
    <>
    <AnimatePresence>{showLikes && <LikesModal video={video} currentUser={currentUser} users={users} onClose={()=>setShowLikes(false)} onFollow={onFollow} followed={followed} onViewProfile={onViewProfile} />}</AnimatePresence>
    <AnimatePresence>{showComments && <CommentsModal video={video} currentUser={currentUser} onClose={()=>setShowComments(false)} showToast={showToast} onViewProfile={onViewProfile} />}</AnimatePresence>
    <AnimatePresence>{showShare && <ShareSheet video={video} currentUser={currentUser} onClose={()=>setShowShare(false)} showToast={showToast} />}</AnimatePresence>
    <AnimatePresence>{showSaveConfirm && <SaveConfirmSheet onClose={()=>setShowSaveConfirm(false)} onViewCollections={()=>{ setShowSaveConfirm(false); showToast?.('Opening collections…','info'); }} />}</AnimatePresence>
    <AnimatePresence>{showMediaActions && (
      <motion.div
        variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
        onClick={()=>setShowMediaActions(false)} style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'rgba(15,10,25,0.5)', display:'flex', alignItems:'flex-end', maxWidth:430, margin:'0 auto' }}>
        <motion.div
          variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
          onClick={e=>e.stopPropagation()} style={{ width:'100%', background:COLORS.surface, borderTopLeftRadius:24, borderTopRightRadius:24, padding:'10px 8px 28px' }}>
    ...
    <AnimatePresence>{showOptions && <PostOptionsMenu video={video} currentUser={currentUser} onClose={()=>setShowOptions(false)} showToast={showToast} onDelete={onDelete} onBlock={onBlock} />}</AnimatePresence>
    <div style={{ background:COLORS.surface, borderRadius:RADIUS.lg, padding:14, marginBottom:0, boxShadow:SHADOW.card, border:`1px solid ${COLORS.border}`, transition:TRANSITION.base }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
        <div onClick={()=>onViewProfile?.(video.userId)} style={{ width:44, height:44, borderRadius:'50%', background:video.avatarColor||COLORS.brand, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:700, fontSize:17, overflow:'hidden', cursor:'pointer', flexShrink:0, position:'relative', border: isLive ? `2px solid ${COLORS.live}` : `2px solid ${COLORS.surface}`, boxShadow: isLive ? SHADOW.glow(COLORS.live) : SHADOW.xs, transition:TRANSITION.fast }}>
          {(author?.avatarUrl || video.avatarUrl) ? <img loading="lazy" decoding="async" src={author?.avatarUrl || video.avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" /> : (displayName||'?')[0]?.toUpperCase()}
          {isLive && (
        ...
        <button onClick={()=>setShowOptions(true)} aria-label="More options" style={{ background:'none', border:'none', cursor:'pointer', color:COLORS.textTertiary, fontSize:16, padding:4, flexShrink:0 }}>•••</button>
      </div>
      ...
      {mediaSrc && (
        ...
        >
          {isVideo ? (
            ...
            <img loading="lazy" decoding="async" src={mediaSrc} alt="" style={{ width:'100%', maxHeight:420, display:'block', objectFit:'cover' }} />
          )}
      ...
      {/* ── Action bar — one unified row, laid out directly on the post's own
          white surface (no separate pill/background/border of its own), so it
          reads as part of the same card instead of a bolted-on widget. Each
          icon carries its own count beside it (Instagram/TikTok-style) instead
      ...
      <div style={{ marginTop:10, display:'flex', alignItems:'center', justifyContent:'space-between', borderTop:`1px solid ${COLORS.border}`, paddingTop:10 }}>
        ...
        <motion.button whileTap={tapScale} onClick={toggleLike} onDoubleClick={()=>setShowLikes(true)} style={{ background:'none', border:'none', padding:8, margin:-8, display:'flex', alignItems:'center', gap:6, cursor:'pointer', transition:TRANSITION.fast, borderRadius:10 }}>
          <svg width="19" height="19" viewBox="0 0 24 24" fill={liked?COLORS.brand:'none'} stroke={liked?COLORS.brand:COLORS.textSecondary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: liked ? 'likeHeart 0.35s ease' : 'none' }}>
            <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>
          </svg>
          <span onClick={e=>{ e.stopPropagation(); likeCount>0 && setShowLikes(true); }} style={{ fontSize:13, color: liked?COLORS.brand:COLORS.textSecondary, cursor: likeCount>0 ? 'pointer' : 'default' }}>{formatNumber(likeCount)}</span>
        </motion.button>

        {/* Comment */}
        <motion.button whileTap={tapScale} onClick={()=>{ setShowComments(true); onOpenComments?.(video); }} style={{ background:'none', border:'none', padding:8, margin:-8, display:'flex', alignItems:'center', gap:6, cursor:'pointer', transition:TRANSITION.fast, borderRadius:10 }}>
          <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke={COLORS.textSecondary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"/></svg>
  ...
  return (
    <div data-main-scroll="true" onScroll={onFeedScroll} style={{ height:'100%', overflowY:'auto', background:COLORS.bg, padding:'10px 16px max(74px, calc(58px + env(safe-area-inset-bottom)))' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
        ...
        <button onClick={onOpenSearch} aria-label="Add friend" style={{ width:38, height:38, borderRadius:'50%', background:COLORS.surfaceAlt, border:'none', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0 }}>
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto', background:COLORS.bg }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'16px 16px 14px', background:COLORS.surface, borderBottom:`1px solid ${COLORS.border}` }}>
        <button onClick={()=>{ if(onClose){ onClose(); } else { setText(''); setMedia([]); } }} aria-label="Close" style={{ background:'none', border:'none', cursor:'pointer', color:COLORS.textTertiary, width:26, height:26, display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <div style={{ padding:16 }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:16, overflowX:'auto' }}>
            {media.map((m,i)=>(
              <div key={i} style={{ position:'relative', width:76, height:76, borderRadius:14, overflow:'hidden', background:COLORS.surfaceAlt, flexShrink:0 }}>
                {m.type?.startsWith('video') ? <video src={m.url} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <img loading="lazy" decoding="async" src={m.url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />}
                <button onClick={()=>removeMedia(i)} aria-label="Remove media" style={{ position:'absolute', top:4, right:4, width:20, height:20, borderRadius:'50%', background:'rgba(0,0,0,0.55)', color:'#fff', border:'none', cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
              </div>
            ))}
            {media.length<4 && (
              <button onClick={()=>fileInputRef.current?.click()} aria-label="Add photo" style={{ width:76, height:76, borderRadius:14, background:COLORS.surfaceAlt, border:`1.5px dashed ${COLORS.borderStrong}`, color:COLORS.brand, fontSize:24, cursor:'pointer', flexShrink:0 }}>+</button>
            )}
          </div>
        )}
        ...
        {showBgPicker && (
          <div style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:16, padding:14, marginBottom:16 }}>
            ...
            {bgColor && media.length>0 && <div style={{ color:COLORS.warningText, fontSize:11.5, marginTop:8 }}>Colored backgrounds only show on text-only posts — this won't appear while photos/videos are attached.</div>}
          </div>
        )}
          ...
          <div style={{ background:COLORS.surface, border:`1px solid ${COLORS.border}`, borderRadius:16, padding:14, marginBottom:16 }}>
            ...
            {gifLoading ? (
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, maxHeight:280, overflowY:'auto' }}>
                {gifResults.map(g=>(
                  <button key={g.id} onClick={()=>pickGif(g)} aria-label="Select GIF" style={{ padding:0, border:'none', borderRadius:10, overflow:'hidden', cursor:'pointer', background:COLORS.surfaceAlt, aspectRatio:'1/1' }}>
                    <img loading="lazy" decoding="async" src={g.images?.fixed_height_small?.url || g.images?.fixed_height?.url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} />
                  </button>
                ))}
...
const EditProfileModal = ({ user, onClose, onSave, showToast }) => {
  ...
  const [avatarColor, setAvatarColor] = useState(user?.avatarColor||COLORS.brand);
      ...
      <div style={{
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6, gap:10 }}>
          <div style={{ display:'flex', gap:10, alignItems:'flex-start', minWidth:0 }}>
            {group.isUserReport ? (
              <div style={{ width:44, height:44, borderRadius:'50%', background:group.targetUser?.avatarColor||COLORS.brand, display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:16, overflow:'hidden', flexShrink:0 }}>
                {group.targetUser?.avatarUrl ? <img loading="lazy" decoding="async" src={group.targetUser.avatarUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/> : (group.targetUser?.avatar || group.label[1] || '?')}
              </div>
            ) : (
              group.thumb ? <img loading="lazy" decoding="async" src={group.thumb} alt="" style={{ width:44, height:44, borderRadius:10, objectFit:'cover', flexShrink:0 }} />
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:6, flexShrink:0 }}>
            {group.urgent && (
              <span style={{ background:`${COLORS.danger}1A`, color:COLORS.dangerText, fontSize:10, fontWeight:800, padding:'3px 8px', borderRadius:10 }}>URGENT</span>
            )}
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginTop:12 }}>
          ...
          {group.reportKind === 'post' && group.targetId && (
            <button onClick={()=>deletePostGroup(group)} style={{ flex:'1 1 110px', background:`${COLORS.danger}1A`, border:`1px solid ${COLORS.danger}4D`, borderRadius:14, padding:'9px', color:COLORS.dangerText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Delete post</button>
          )}

          {group.reportKind === 'comment' && group.targetId && (
            <button onClick={()=>deleteCommentGroup(group)} style={{ flex:'1 1 110px', background:`${COLORS.danger}1A`, border:`1px solid ${COLORS.danger}4D`, borderRadius:14, padding:'9px', color:COLORS.dangerText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Delete comment</button>
          )}

          {group.isUserReport && group.targetId && (
            <>
              <button onClick={()=>warnUser(group)} style={{ flex:'1 1 90px', background:`${COLORS.warning}1A`, border:`1px solid ${COLORS.warning}4D`, borderRadius:14, padding:'9px', color:COLORS.warningText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Warn</button>

              <div style={{ position:'relative', flex:'1 1 110px' }}>
                <button onClick={()=>setOpenDurationFor(durationOpen?null:group.key)} style={{ width:'100%', background:`${COLORS.warning}1A`, border:`1px solid ${COLORS.warning}4D`, borderRadius:14, padding:'9px', color:COLORS.warningText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Suspend ▾</button>
                {durationOpen && (
              ...
              <button onClick={()=>banUser(group)} style={{ flex:'1 1 90px', background:`${COLORS.danger}1A`, border:`1px solid ${COLORS.danger}4D`, borderRadius:14, padding:'9px', color:COLORS.dangerText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Ban</button>

              {(group.targetUser?.accountStatus==='suspended' || group.targetUser?.accountStatus==='banned' || group.targetUser?.accountStatus==='warned') && (
                <button onClick={()=>reinstateUser(group)} style={{ flex:'1 1 110px', background:`${COLORS.success}1A`, border:`1px solid ${COLORS.success}4D`, borderRadius:14, padding:'9px', color:COLORS.successText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Reinstate</button>
              )}
            </>
          )}
        </div>
            ...
            <div key={v.id} style={{ background:COLORS.surface2, borderRadius:18, padding:'14px 16px', marginBottom:10, border:`1px solid ${COLORS.border}` }}>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginBottom:10 }}>
                {(v.moderationCategories?.length ? v.moderationCategories : ['review']).map(cat => (
                  <span key={cat} style={{ background:`${COLORS.warning}1A`, color:COLORS.warningText, fontSize:10.5, fontWeight:700, padding:'3px 9px', borderRadius:10 }}>{cat}</span>
                ))}
              </div>
              <div style={{ display:'flex', gap:8 }}>
                <button onClick={()=>reviewFlagged(v.id,'approve')} style={{ flex:1, background:`${COLORS.success}1A`, border:`1px solid ${COLORS.success}4D`, borderRadius:14, padding:'9px', color:COLORS.successText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Approve</button>
                <button onClick={()=>reviewFlagged(v.id,'reject')} style={{ flex:1, background:`${COLORS.danger}1A`, border:`1px solid ${COLORS.danger}4D`, borderRadius:14, padding:'9px', color:COLORS.dangerText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Remove</button>
              </div>
            </div>
          ))}
        </>
          ...
          {appeals.map(a => {
            ...
            return (
              <div key={a.id} style={{
                ...
                {a.status === 'pending' && (
                  <div style={{ display:'flex', gap:8 }}>
                    <button onClick={()=>approveAppeal(a)} style={{ flex:1, background:`${COLORS.success}1A`, border:`1px solid ${COLORS.success}4D`, borderRadius:14, padding:'9px', color:COLORS.successText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Approve &amp; reinstate</button>
                    <button onClick={()=>denyAppeal(a)} style={{ flex:1, background:`${COLORS.danger}1A`, border:`1px solid ${COLORS.danger}4D`, borderRadius:14, padding:'9px', color:COLORS.dangerText, fontWeight:700, fontSize:12.5, cursor:'pointer' }}>Deny</button>
                  </div>
                )}
              </div>
            );
      ...
      {(user?.blockedUsers||[]).map(uid=>{
        ...
        return (
          <div key={uid} style={{ display:'flex', alignItems:'center', gap:12, background:COLORS.surface2, borderRadius:18, padding:'14px 16px', marginBottom:10, border:`1px solid ${COLORS.border}` }}>
            ...
            }} style={{ background:`${COLORS.danger}1A`, border:`1px solid ${COLORS.danger}4D`, borderRadius:20, padding:'8px 16px', color:COLORS.dangerText, fontWeight:700, fontSize:13, cursor:'pointer', flexShrink:0 }}>Unblock</button>
          </div>
        );
      })}
    </div>
...
unsub = onSnapshot(q, (snap) => {
  ...
  const handleSend = async (overrideText) => {
    ...
    // most direct place for one user to harass another. Fail open on a
    // moderation-service hiccup so a network blip never blocks a legitimate message.
    if (msg) {
      try {
        const modResult = await apiFetch('/api/moderation/check', { method: 'POST', body: JSON.stringify({ text: msg }) });
        if (modResult?.blocked) {
          showToast?.('This message looks like it may violate our community guidelines and was not sent.', 'error');
          return;
        }
...
{showEmoji && (
      ...
      <AnimatePresence>{showChatInfo && (
        <motion.div
          variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
          onClick={()=>setShowChatInfo(false)} style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'rgba(11,15,25,0.55)', display:'flex', alignItems:'flex-end' }}>
          <motion.div
            variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
            onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:430, margin:'0 auto', maxHeight:'85vh', overflowY:'auto', background:COLORS.surface, borderTopLeftRadius:26, borderTopRightRadius:26, paddingBottom:'max(24px, env(safe-area-inset-bottom))' }}>
            ...
            <div style={{ padding:'8px 8px' }}>
              ...
              <button onClick={blockThisUser} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, background:'none', border:'none', borderTop:`1px solid ${COLORS.border}`, marginTop:6, paddingTop:16, padding:'16px 14px 13px', color:COLORS.dangerText, fontSize:14.5, fontWeight:600, cursor:'pointer', textAlign:'left' }}>
              ...
              <button onClick={()=>setShowReportUserSheet(true)} style={{ width:'100%', display:'flex', alignItems:'center', gap:12, background:'none', border:'none', padding:'13px 14px', color:COLORS.dangerText, fontSize:14.5, fontWeight:600, cursor:'pointer', textAlign:'left' }}>
      ...
      <AnimatePresence>{showReportUserSheet && (
        <ReportReasonSheet
          title="Report User"
      ...
      <AnimatePresence>{showThemeSheet && (
        <motion.div
          variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
          onClick={()=>setShowThemeSheet(false)} style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'rgba(11,15,25,0.55)', display:'flex', alignItems:'flex-end' }}>
          <motion.div
            variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
            onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:430, margin:'0 auto', maxHeight:'80vh', overflowY:'auto', background:COLORS.surface, borderTopLeftRadius:26, borderTopRightRadius:26, paddingBottom:'max(24px, env(safe-area-inset-bottom))' }}>
            <div style={{ width:36, height:4, background:COLORS.border, borderRadius:2, margin:'12px auto 8px' }} />
      ...
      <AnimatePresence>{showDisappearingSheet && (
        <motion.div
          variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
          onClick={()=>setShowDisappearingSheet(false)} style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'rgba(11,15,25,0.55)', display:'flex', alignItems:'flex-end' }}>
          <motion.div
            variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
            onClick={e=>e.stopPropagation()} style={{ width:'100%', maxWidth:430, margin:'0 auto', maxHeight:'80vh', overflowY:'auto', background:COLORS.surface, borderTopLeftRadius:26, borderTopRightRadius:26, paddingBottom:'max(24px, env(safe-area-inset-bottom))' }}>
            <div style={{ width:36, height:4, background:COLORS.border, borderRadius:2, margin:'12px auto 8px' }} />
    ...
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:COLORS.bg, position:'relative' }}>
      <div style={{ padding:'14px 16px 0', background:COLORS.surface, borderBottom:`1px solid ${COLORS.border}` }}>
        ...
        <div style={{ display:'flex', alignItems:'center', background:COLORS.surfaceAlt, borderRadius:14, padding:'9px 12px', gap:8, marginBottom:12 }}>
          ...
          {inboxSearch && (
            <button onClick={()=>setInboxSearch('')} aria-label="Clear search" style={{ background:'none', border:'none', cursor:'pointer', color:COLORS.textTertiary, display:'flex', padding:2 }}>
      ...
      <AnimatePresence>{actionSheetUser && (
        <motion.div
          variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
          onClick={()=>setActionSheetUser(null)} style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'rgba(11,15,25,0.45)', display:'flex', alignItems:'flex-end' }}>
          <motion.div
            variants={sheetVariants} initial="hidden" animate="visible" exit="exit"
...
const IncomingCallScreen = ({ callData, onAnswer, onDecline }) => {
  ...
  return (
  <div style={{position:'fixed',inset:0,background:'linear-gradient(160deg,#0d0025,#001a0d)',zIndex:Z.call,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'space-between',padding:'80px 40px 80px'}}>
    <div style={{textAlign:'center'}}>
      ...
      <div style={{width:120,height:120,borderRadius:'50%',background:callData.callerColor||COLORS.brand,display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontWeight:'bold',fontSize:48,margin:'0 auto 20px',border:'4px solid rgba(255,255,255,0.2)',boxShadow:'0 0 0 12px rgba(255,255,255,0.05),0 0 0 24px rgba(255,255,255,0.03)'}}>
    ...
    <div style={{display:'flex',justifyContent:'space-around',width:'100%',alignItems:'center'}}>
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
        <button onClick={onDecline} aria-label="Decline call" style={{width:70,height:70,borderRadius:'50%',background:COLORS.danger,border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',boxShadow:'0 8px 30px rgba(239,68,68,0.5)'}}>
      ...
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
        <button onClick={onAnswer} aria-label="Answer call" style={{width:70,height:70,borderRadius:'50%',background:'#2ED573',border:'none',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',boxShadow:'0 8px 30px rgba(52,199,89,0.5)',animation:'pulse 1.5s infinite'}}>
...
cleanupCall();
  ...
  if (type !== 'video') {
    ...
    return (
      <>
        ...
        {/* The pill/card swap below is a state toggle within one already-mounted
            component (not a mount/unmount), so it gets its own AnimatePresence
            rather than relying on the one at the call site — mode="wait" keeps
            the two from cross-fading into each other mid-swap. modalVariants on
            the card replaces the bespoke popInBounce keyframe so a call popup
            and a confirm dialog share the same entrance physics; springs.bouncy
            stays reserved for celebrations per motion.js. */}
        ...
        {minimized ? (
          ...
          <motion.div key="pill" variants={fadeVariants} initial="hidden" animate="visible" exit="exit"
            onClick={()=>setMinimized(false)} style={{ position:'fixed', top:'max(14px, env(safe-area-inset-top))', left:'50%', transform:'translateX(-50%)', zIndex:Z.call, background:COLORS.gradient, borderRadius:999, padding:'7px 8px 7px 14px', display:'flex', alignItems:'center', gap:10, boxShadow:SHADOW.raised, cursor:'pointer' }}>
          ...
          <motion.div key="card" variants={modalVariants} initial="hidden" animate="visible" exit="exit"
            style={{ position:'fixed', top:'max(20px, env(safe-area-inset-top))', right:16, zIndex:Z.call, width:264, background:COLORS.surface, borderRadius:RADIUS.xl, boxShadow:SHADOW.raised, border:`1px solid ${COLORS.border}`, padding:'16px 16px 18px', display:'flex', flexDirection:'column', alignItems:'center' }}>
            ...
            <div style={{ width:76, height:76, borderRadius:'50%', padding:3, background:COLORS.gradient, margin:'4px auto 12px', animation:status==='calling'?'storyRing 4s linear infinite':'' }}>
              <div style={{ width:'100%', height:'100%', borderRadius:'50%', background:COLORS.surface, padding:2, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <div style={{ width:'100%', height:'100%', borderRadius:'50%', background:COLORS.brandTextFill, display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontWeight:700, fontSize:28 }}>
                  {contactAvatar || '?'}
                </div>
              </div>
            </div>
            ...
            <div style={{ display:'flex', justifyContent:'center', gap:16 }}>
              <button onClick={toggleMute} aria-label={isMuted ? "Unmute microphone" : "Mute microphone"} style={{ background:isMuted?COLORS.danger:COLORS.surfaceAlt, border:'none', borderRadius:'50%', width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
  ...
  return (
    <motion.div
      variants={pageVariants} initial="hidden" animate="visible" exit="exit"
      style={{ position:'fixed', inset:0, background:'#0B0B0F', zIndex:Z.call, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ position:'absolute', bottom:60, left:0, right:0, zIndex:20, display:'flex', justifyContent:'center', gap:20 }}>
        <button onClick={toggleMute} aria-label={isMuted ? "Unmute microphone" : "Mute microphone"} style={{ background:isMuted?COLORS.danger:'rgba(255,255,255,0.12)', border:'none', borderRadius:'50%', width:60, height:60, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
        ...
        <button onClick={onClose} aria-label="End call" style={{ background:COLORS.brand, border:'none', borderRadius:'50%', width:70, height:70, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', boxShadow:'0 0 30px rgba(11,95,255,0.5)' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5">
            <path d="M10.68 13.31a16 16 0 003.41 2.6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7 2 2 0 011.72 2v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.42 19.42 0 01-3.33-2.67m-2.67-3.34a19.79 19.79 0 01-3.07-8.63A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91"/>
            <line x1="23" y1="1" x2="1" y2="23"/>
          </svg>
        </button>
        <button onClick={toggleCam} aria-label={isCamOff ? "Turn camera on" : "Turn camera off"} style={{ background:isCamOff?COLORS.danger:'rgba(255,255,255,0.12)', border:'none', borderRadius:'50%', width:60, height:60, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
...
const SearchOverlay = ({ onClose, videos, users, onViewProfile, currentUser, onFollow, followed }) => {
  ...
  return (
    <div style={{ position:'absolute', inset:0, background:COLORS.bg, zIndex:Z.page, display:'flex', flexDirection:'column' }}>
      {/* ── Header ── */}
      <div style={{ padding:'12px 16px', borderBottom:`1px solid ${COLORS.overlaySubtle}`, display:'flex', gap:10, alignItems:'center', background:COLORS.surface, backdropFilter:'blur(20px)' }}>
        <div style={{ flex:1, display:'flex', alignItems:'center', background:COLORS.border, borderRadius:14, padding:'11px 14px', gap:10, border:`1.5px solid ${COLORS.border}`, transition:'border-color 0.2s' }}>
          ...
          {query && <button onClick={()=>setQuery('')} aria-label="Clear search" style={{ background:COLORS.border, border:'none', borderRadius:'50%', width:20, height:20, color:COLORS.textTertiary, cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>✕</button>}
        </div>
              ...
              <div style={{ marginTop:20, marginBottom:24 }}>
                ...
                {recentSearches.map((term,i)=>(
                  <div key={i} onClick={()=>setQuery(term)} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0', borderBottom:`1px solid ${COLORS.overlaySubtle}`, cursor:'pointer' }}>
                    ...
                    <button onClick={e=>{e.stopPropagation(); const u=[...recentSearches]; u.splice(i,1); setRecentSearches(u); try{localStorage.setItem('Infinity_recent_searches',JSON.stringify(u));}catch{}}} aria-label="Remove recent search" style={{ background:'none', border:'none', color:COLORS.textTertiary, cursor:'pointer', fontSize:16 }}>✕</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ padding:'8px 14px' }}>
            ...
            {(tab==='all'||tab==='users') && results.users.length > 0 && (
              <div style={{ marginBottom:20 }}>
                ...
                <motion.div variants={listContainerVariants} initial="hidden" animate="visible">
                {results.users.map(u=>(
                  <motion.div key={u.id} variants={listItemVariants} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 12px', background:COLORS.overlaySubtle, borderRadius:16, marginBottom:6, border:`1px solid ${COLORS.overlaySubtle}`, transition:'background 0.1s' }}>
              ...
              <div style={{ marginBottom:20 }}>
                ...
                <motion.div variants={listContainerVariants} initial="hidden" animate="visible" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  {results.videos.slice(0,6).map(v=>(
                    <motion.div key={v.id} variants={listItemVariants} style={{ aspectRatio:'9/16', position:'relative', borderRadius:14, overflow:'hidden', background:COLORS.surfaceAlt, cursor:'pointer' }}>
                      {v.videoUrl?.match(/\.(jpg|jpeg|png|gif|webp)/i) || v.mediaType?.startsWith('image')
                        ? <img loading="lazy" decoding="async" src={v.videoUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                        : <video src={v.videoUrl} style={{ width:'100%', height:'100%', objectFit:'cover' }} muted />}
                  ...
                  <div key={v.id} style={{ padding:'12px 14px', background:COLORS.overlaySubtle, borderRadius:14, marginBottom:6, border:`1px solid ${COLORS.overlaySubtle}`, cursor:'pointer' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
                      <div style={{ width:28, height:28, borderRadius:'50%', background:users.find(u=>u.id===v.userId)?.avatarColor||COLORS.brand, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:12, fontWeight:'bold' }}>
                        {users.find(u=>u.id===v.userId)?.avatarUrl ? <img loading="lazy" decoding="async" src={users.find(u=>u.id===v.userId).avatarUrl} style={{width:'100%',height:'100%',objectFit:'cover'}} alt="" /> : v.username?.[0]?.toUpperCase()}
                      </div>
...
const CameraUpload = ({ onUpload, onClose, showToast, currentUser }) => {
  ...
  if(selectedFile) return (
    <div style={{ position:'fixed', inset:0, background:'#000', zIndex:Z.page, display:'flex', flexDirection:'column' }}>
      <div style={{ padding:'14px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <button onClick={handleUpload} disabled={uploading} style={{ background:COLORS.gradient, border:'none', borderRadius:20, padding:'8px 18px', color:'white', fontWeight:700, cursor:'pointer', fontSize:13, opacity:uploading?0.7:1 }}>
          {uploading ? `${uploadProgress}%` : 'Post ✓'}
        </button>
      </div>
      {uploading && <div style={{ height:3, background:'rgba(255,255,255,0.1)' }}><div style={{ height:'100%', background:`linear-gradient(90deg,${COLORS.brandLight},${COLORS.brand})`, width:`${uploadProgress}%`, transition:'width 0.3s' }} /></div>}
          ...
          <div style={{ display:'flex', width:'100%', height:'100%', overflowX:'auto', scrollSnapType:'x mandatory' }}>
            {selectedFile.urls.map((u,i)=>(
              <div key={i} style={{ flex:'0 0 100%', height:'100%', scrollSnapAlign:'center' }}>
                <img loading="lazy" decoding="async" src={u} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', ...filterStyle }} />
              </div>
            ))}
          </div>
        ) : selectedFile.type.startsWith('image/') 
          ? <img loading="lazy" decoding="async" src={selectedFile.url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', ...filterStyle }} />
      ...
      <div style={{ padding:'10px 0', background:'rgba(0,0,0,0.8)', overflowX:'auto', display:'flex', gap:10, paddingLeft:16 }}>
        {FILTERS.map((f,i)=>(
          <div key={f.name} onClick={()=>setActiveFilter(i)} style={{ flexShrink:0, textAlign:'center', cursor:'pointer' }}>
            <div style={{ width:56, height:56, borderRadius:14, overflow:'hidden', border: i===activeFilter?`2px solid ${COLORS.brand}`:'2px solid transparent' }}>
              <img loading="lazy" decoding="async" src={selectedFile.type==='image/multi' ? selectedFile.urls[0] : selectedFile.type.startsWith('image/')?selectedFile.url:'https://picsum.photos/56'} alt="" style={{ width:'100%', height:'100%', objectFit:'cover', filter: f.css||'none' }} />
            </div>
            <div style={{ color: i===activeFilter?COLORS.brand:'rgba(255,255,255,0.5)', fontSize:9, marginTop:4, fontWeight:700 }}>{f.name}</div>
          </div>
        ))}
      </div>
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'#000', zIndex:Z.page, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ position:'absolute', top:0, left:0, right:0, zIndex:10, padding:'50px 16px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <button onClick={onClose} aria-label="Close" style={{ background:'rgba(0,0,0,0.5)', border:'none', borderRadius:'50%', width:40, height:40, color:'white', cursor:'pointer', fontSize:20, display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
      ...
      <div onClick={handleFocusTap} style={{ flex:1, position:'relative', overflow:'hidden', cursor: cameraMode==='photo' ? 'default' : 'default' }}>
        ...
        {zoomCaps && (
          <div onClick={e=>e.stopPropagation()} style={{ position:'absolute', right:14, top:'50%', transform:'translateY(-50%)', display:'flex', flexDirection:'column', alignItems:'center', gap:8, background:'rgba(0,0,0,0.45)', borderRadius:20, padding:'12px 6px' }}>
            ...
            <input
              ...
              style={{ writingMode:'vertical-lr', WebkitAppearance:'slider-vertical', height:120, width:24, accentColor:COLORS.brand }}
            />
          </div>
        )}
        {/* Live filter strip */}
        {showFilters && (
          <div onClick={e=>e.stopPropagation()} style={{ position:'absolute', bottom:0, left:0, right:0, padding:'10px 0 10px 16px', background:'linear-gradient(transparent,rgba(0,0,0,0.7))', overflowX:'auto', display:'flex', gap:10 }}>
            {FILTERS.map((f,i)=>(
              <div key={f.name} onClick={()=>setActiveFilter(i)} style={{ flexShrink:0, textAlign:'center', cursor:'pointer' }}>
                <div style={{ width:52, height:52, borderRadius:12, background:'rgba(255,255,255,0.15)', border: i===activeFilter?`2px solid ${COLORS.brand}`:'2px solid transparent', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' }}>
      ...
      <div style={{ background:'rgba(0,0,0,0.9)', padding:'20px 0 48px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:28, marginBottom: cameraMode==='video' && !recording ? 12 : 24 }}>
          {['photo','video'].map(m=>(
            <button key={m} onClick={()=>{
              ...
              setCameraMode(m);
            }} style={{ background:'none', border:'none', color: cameraMode===m?'white':'rgba(255,255,255,0.35)', fontSize:13, fontWeight:700, cursor:'pointer', textTransform:'uppercase', letterSpacing:1, borderBottom: cameraMode===m?`2px solid ${COLORS.brand}`:'2px solid transparent', paddingBottom:4 }}>{m}</button>
          ))}
        </div>
        ...
        {cameraMode==='video' && !recording && (
          <div style={{ display:'flex', justifyContent:'center', gap:10, marginBottom:24 }}>
            {[0.5,1,2,3].map(sp=>(
              <button key={sp} onClick={()=>setRecordSpeed(sp)} style={{ background: recordSpeed===sp?'rgba(11,95,255,0.25)':'rgba(255,255,255,0.08)', border: recordSpeed===sp?`1px solid ${COLORS.brand}`:'1px solid rgba(255,255,255,0.1)', borderRadius:16, padding:'5px 12px', color: recordSpeed===sp?COLORS.brand:'rgba(255,255,255,0.6)', fontSize:12, fontWeight:700, cursor:'pointer' }}>{sp}x</button>
            ))}
          </div>
        )}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, padding:'0 16px 14px', overflowX:'auto' }}>
            {capturedPhotos.map((p,i)=>(
              <div key={i} style={{ position:'relative', flexShrink:0 }}>
                <img loading="lazy" decoding="async" src={p.url} alt="" style={{ width:52, height:52, borderRadius:10, objectFit:'cover', border:'2px solid rgba(255,255,255,0.3)' }} />
                <button onClick={()=>removeCapturedPhoto(i)} aria-label="Remove photo" style={{ position:'absolute', top:-6, right:-6, width:18, height:18, borderRadius:'50%', background:'rgba(0,0,0,0.8)', border:'1px solid rgba(255,255,255,0.4)', color:'white', fontSize:11, lineHeight:1, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
              </div>
            ))}
            <button onClick={finishPhotoCapture} aria-label="Confirm photo" style={{ flexShrink:0, width:52, height:52, borderRadius:'50%', background:COLORS.gradient, border:'none', color:'white', fontSize:20, fontWeight:800, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>✓</button>
          </div>
        )}
        ...
        {cameraMode==='video' && !recording && segments.length > 0 && (
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:16, padding:'0 16px 14px' }}>
            ...
            <button onClick={finishVideoRecording} style={{ background:COLORS.gradient, border:'none', borderRadius:16, padding:'6px 16px', color:'white', fontSize:12, fontWeight:800, cursor:'pointer' }}>Done ✓</button>
          </div>
        )}
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingLeft:60, paddingRight:60 }}>
          {/* Gallery */}
          <button onClick={()=>fileInputRef.current?.click()} aria-label="Choose from gallery" style={{ width:48, height:48, borderRadius:14, background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:22 }}>🖼️</button>
          <input ref={fileInputRef} type="file" accept="video/*,image/*" multiple onChange={handleFileSelect} style={{ display:'none' }} />

          {/* Shutter / Record */}
          {cameraMode==='photo' ? (
            <button onClick={capturePhoto} aria-label="Take photo" disabled={countdown>0} style={{ width:76, height:76, borderRadius:'50%', background:'white', border:'5px solid rgba(255,255,255,0.3)', cursor: countdown>0 ? 'default' : 'pointer', opacity: countdown>0 ? 0.5 : 1, position:'relative' }}>
              <div style={{ position:'absolute', inset:4, borderRadius:'50%', background:'white' }} />
            </button>
          ) : (
            <button onClick={recording?stopRecording:startRecording} disabled={countdown>0} style={{ width:76, height:76, borderRadius:'50%', background: recording?COLORS.brand:'white', border:'5px solid rgba(255,255,255,0.3)', cursor: countdown>0 ? 'default' : 'pointer', opacity: countdown>0 ? 0.5 : 1, display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
              {recording && (
                <svg width="86" height="86" viewBox="0 0 86 86" style={{ position:'absolute', top:-5, left:-5, transform:'rotate(-90deg)', pointerEvents:'none' }}>
                  ...
                  <circle cx="43" cy="43" r="40" fill="none" stroke={COLORS.brand} strokeWidth="3"
              ...
              {recording
                ...
                : <div style={{ width:76, height:76, borderRadius:'50%', background:COLORS.brand }} />
              }
            </button>
          )}

          {/* Flip (right side placeholder for symmetry) */}
          <button onClick={flipCamera} aria-label="Flip camera" style={{ width:48, height:48, borderRadius:'50%', background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.15)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontSize:22 }}>🔄</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'0 14px' }}>
        {filtered.map(sound=>(
          <div key={sound.id} onClick={()=>onSelectSound(sound)} style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 12px', background:COLORS.overlaySubtle, borderRadius:18, marginBottom:8, cursor:'pointer', border:`1px solid ${COLORS.overlaySubtle}` }}>
            ...
            {sound.popular && <span style={{ color:COLORS.warningText, fontSize:11, fontWeight:700 }}>🔥 {formatNumber(sound.usage)}</span>}
          </div>
        ))}
      </div>
    </div>
...
const CreatorAnalytics = ({ user, videos, onClose }) => {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:COLORS.bg, zIndex:Z.page, overflow:'auto' }}>
          ...
          <div style={{ height:120, display:'flex', alignItems:'flex-end', gap:6 }}>
            {weeklyData.map((v,i)=>(
              <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                <div style={{ width:'100%', height:`${Math.max((v/maxVal)*100,4)}%`, background:`linear-gradient(180deg,${COLORS.brandLight},${COLORS.brand})`, borderRadius:6 }} />
  ...
  return (
    <div style={{ height:'100%', position:'relative', overflow:'hidden', background:'#000' }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      {videos.map((video,idx)=>(
        <div key={video.id} onClick={()=>setMuted(m=>!m)} style={{ position:'absolute', inset:0, transform:`translateY(${(idx-currentIndex)*100}%)`, transition:'transform 0.3s cubic-bezier(0.25,0.46,0.45,0.94)', pointerEvents:idx===currentIndex?'auto':'none' }}>
          {video.videoUrl?.match(/\.(jpg|jpeg|png|gif|webp)/i)
            ? <img loading="lazy" decoding="async" src={video.videoUrl} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
      ...
      <div style={{ position:'absolute', bottom:28, left:0, right:0, display:'flex', justifyContent:'center', zIndex:20 }}>
        <button onClick={onSignIn} style={{ background:COLORS.gradient, border:'none', borderRadius:28, padding:'14px 36px', color:'white', fontWeight:800, fontSize:15, cursor:'pointer', boxShadow:'0 4px 24px rgba(11,95,255,0.5)' }}>
          Sign in to interact 🚀
        </button>
      </div>
    </div>
...
const NotificationsPage = ({ currentUser, users, videos, onClose, onViewProfile, onFollow, followed, showToast, t, onNavigate }) => {
  ...
  // Was missing an error callback entirely — a permission or index failure would
  // just leave notifs at [] with no signal, which renders identically to the
  // "No notifications yet" empty state below. Same failure mode as the Feed's
  // videosError fix; give it the same explicit error branch + retry.
  const [notifsError, setNotifsError] = useState(false);
  const [notifsRetryKey, setNotifsRetryKey] = useState(0);
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:COLORS.bg, zIndex:Z.page, display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, overflowY:'auto' }}>
        {notifsLoading ? (
          <div style={{padding:'12px 16px'}}><SkeletonLoader count={6} /></div>
        ) : notifsError ? (
          <div style={{ textAlign:'center', padding:'60px 20px', color:COLORS.textTertiary }}>
            <div style={{ fontSize:44, marginBottom:12 }}>⚠️</div>
            <div style={{ fontSize:15, fontWeight:600, color:COLORS.textSecondary }}>Couldn't load notifications</div>
            <div style={{ fontSize:13, marginTop:6, marginBottom:16 }}>This is usually a permissions or connection issue.</div>
            <button onClick={()=>setNotifsRetryKey(k=>k+1)} style={{ background:COLORS.gradient, border:'none', borderRadius:20, padding:'10px 22px', color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer' }}>Retry</button>
          </div>
        ) : (
          <>
  ...
  return (
    <div data-main-scroll="true" onScroll={onFeedScroll} style={{ height:'100%', overflowY:'auto', background:COLORS.bg, padding:'10px 14px max(74px, calc(58px + env(safe-area-inset-bottom)))', position:'relative' }}>
      ...
      {!videosLoading && videosError && !filteredVideos.length && (
        <div style={{ textAlign:'center', padding:'60px 20px', color:COLORS.textTertiary }}>
          <div style={{ fontSize:44, marginBottom:10 }}>⚠️</div>
          <div style={{ marginBottom:14 }}>Couldn't load posts. Check your connection and try again.</div>
          <button onClick={()=>window.location.reload()} style={{ background:COLORS.gradient, border:'none', borderRadius:14, padding:'9px 20px', color:'white', fontSize:13, fontWeight:700, cursor:'pointer' }}>
            Retry
          </button>
        </div>
...
export default function InfinityV1app() {
  ...
  // Queue, not a single slot: if a Firestore notification and an FCM push both
  // land within the same second (or two Firestore notifications land back to
  // back), the second one used to silently overwrite the first before its
  // 4.5s timer even ran. notifQueue holds everything waiting to be shown;
  // notifPopup (derived below) is just the head of that queue.
  const [notifQueue, setNotifQueue] = useState([]); // [{ notif, user }, ...]
  ...
  useEffect(()=>{
    ...
    try {
      const unsub = onMessage(messaging, payload=>{
        ...
        const fromUserId = payload?.data?.fromUserId || null;
        const fromUser = fromUserId ? usersRef.current.find(u=>u.id===fromUserId) : null;
        // Same rich, tappable, swipe-to-dismiss popup used for in-app
        // notifications (like/comment/follow/etc) — this used to fall back to
        // the plain generic Toast, which had no avatar and nothing to tap.
        enqueueNotifPopup({
          notif: { type, message: body || title, fromUserId, callType: payload?.data?.callType },
          user: fromUser || { username: payload?.data?.fromUsername || 'someone' },
        });
    ...
    const unsub = onSnapshot(q, snap=>{
      ...
      snap.docChanges().forEach(change=>{
        if(change.type==='added'){
          ...
          enqueueNotifPopup({ notif:{...data,id:change.doc.id}, user:fromUser });
        }
      });
    },()=>{});
    return ()=>unsub();
...
const handleMessage = uid => {
  ...
  if(authLoading) return (
    <div style={{ maxWidth:430, margin:'0 auto', height:'100dvh', background:COLORS.bg, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:16 }}>
      ...
      <img loading="lazy" decoding="async" src="https://res.cloudinary.com/dotvhzjmc/image/upload/znfksngv27boh3c1kxpv.png" style={{ width:80, height:80, borderRadius:24, marginBottom:16 }} alt="Infinity" />
    ...
    <div style={{ maxWidth:430, margin:'0 auto', height:'100dvh', background:'#0B0B0F', overflow:'hidden' }}>
      ...
      {notifPopup && (
        <NotifPopup
          ...
          onTap={()=>{ openNotifTarget(notifPopup.notif); dismissNotifPopup(); }}
        />
      )}
...
{incomingCall && !showCall && (
  ...
  setShowCall({
    ...
    callDocId: snap.callDocId || snap.id
      ...
      <AnimatePresence>{showCall && (
        <CallModal type={showCall.type} contactName={showCall.contactName} contactAvatar={showCall.contactAvatar} contactId={showCall.contactId} currentUser={currentUser} onClose={()=>setShowCall(null)} isCallee={showCall.isCallee} callDocId={showCall.callDocId} showToast={showToast} />
      )}</AnimatePresence>
      ...
      <AnimatePresence>{showCreateStory && (
        <CreateStoryModal currentUser={currentUser} onClose={()=>setShowCreateStory(false)} showToast={showToast} />
      )}</AnimatePresence>
      {showStoriesPage && (
        <StoriesPage
          users={users}
 ...
 isLive={liveStreamerIds?.has(viewingProfile?.id)} onJoinLive={u=>{ setShowLiveStream(u); setViewingProfile(null); }} />
      ...
      <AnimatePresence>{showProfileDrawer && (
        <>
          <motion.div
            variants={backdropVariants} initial="hidden" animate="visible" exit="hidden"
            onClick={()=>setShowProfileDrawer(false)} style={{ position:'fixed', inset:0, zIndex:Z.drawer, background:'rgba(30,27,46,0.4)', backdropFilter:'blur(2px)', maxWidth:430, margin:'0 auto' }} />
          <motion.div
            variants={drawerVariants} initial="hidden" animate="visible" exit="exit"
            style={{ position:'fixed', top:0, bottom:0, left:'max(0px, calc(50% - 215px))', width:'86%', maxWidth:370, zIndex:Z.drawer + 1, background:COLORS.bg, boxShadow:'8px 0 40px rgba(30,27,46,0.25)', overflow:'hidden' }}>
      ...
      {quickConversation && (
        <div style={{ position:'fixed', inset:0, zIndex:Z.modal, background:'#0B0B0F', maxWidth:430, margin:'0 auto' }}>
          <ConversationView
            currentUser={currentUser}
      ...
      <div ref={contentWrapperRef} onTouchStart={handleTabTouchStart} onTouchEnd={handleTabTouchEnd} style={{ flex:1, overflow:'hidden', position:'relative', minHeight:0, height:'100%' }}>
        ...
        {showTextComposer && (
          <div style={{ position:'fixed', inset:0, zIndex:Z.modal, background:COLORS.bg, maxWidth:430, margin:'0 auto' }}>
