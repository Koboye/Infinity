...
import { GenerationHome } from './Generation';
...
{incomingCall && !showCall && (
 ...
 followed={followed} showToast={showToast} userVideos={videos.filter(v=>v.userId===viewingProfile?.id)} />
      ...
      <div style={{ flex:1, overflow: isGenHome ? 'auto' : 'hidden', position:'relative', minHeight:0 }}>
        ...
        {!showSearch && !showCamera && (
          <>
            {activeTab==='home' && <GenerationHome
  currentUser={currentUser} users={users} videos={videos} followed={followed} onFollow={toggleFollow}
  showToast={showToast} onViewProfile={handleViewProfile} setActiveTab={setActiveTab}
  onOpenExplore={()=>setShowDiscover(true)} onOpenNotifications={()=>setShowNotifications(true)}
  onOpenBookmarks={()=>setShowSavedPosts(true)} onOpenCreateStory={()=>setShowCreateStory(true)}
  onOpenStoryViewer={setShowStoryViewer} isDark={isDark} toggleTheme={toggleTheme}
  db={db} uploadToCloudinary={uploadToCloudinary} sendNotification={sendNotification}
  notifCount={notifBadgeCount} msgCount={0}
/>}
