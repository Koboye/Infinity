...
export function CommentsSheet({ videoId, videoOwnerId, videoOwnerUsername, onClose }: CommentsSheetProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        position: 'relative', background: '#1C1C24',
        ...
        <div style={{
          ...
          <span style={{ fontWeight: 700, fontSize: 16, color: 'white' }}>💬 {comments.length} Comments</span>
            ...
            <div key={c.id} style={{ display: 'flex', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                  ...
                  {c.pinned && (
                    <span style={{ fontSize: 10, background: 'rgba(107,78,255,0.15)', color: '#6B4EFF', borderRadius: 4, padding: '1px 6px', fontWeight: 700 }}>📌 Pinned</span>
                  )}
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginLeft: 'auto' }}>{timeAgo(c.createdAt)}</span>
                </div>
                <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.85)', margin: 0, lineHeight: 1.4 }}>{c.text}</p>
                <div style={{ display: 'flex', gap: 14, marginTop: 6 }}>
                  <button onClick={() => likeComment(c)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
                    ❤️ {c.likes > 0 ? c.likes : ''}
                  </button>
                  <button onClick={() => { setReplyTo(c); inputRef.current?.focus(); }} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: 12, cursor: 'pointer' }}>
                    Reply
                  </button>
                </div>
              </div>
        ...
        {replyTo && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px', background: 'rgba(107,78,255,0.08)', borderTop: '1px solid rgba(107,78,255,0.15)' }}>
            <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>
              Replying to <span style={{ color: '#6B4EFF', fontWeight: 700 }}>@{replyTo.username}</span>
            </span>
        ...
        <div style={{ display: 'flex', gap: 10, padding: '12px 16px 32px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button
            ...
            style={{ background: text.trim() ? 'linear-gradient(135deg,#6B4EFF,#9D4EDD)' : 'rgba(255,255,255,0.07)', border: 'none', color: 'white', borderRadius: 999, width: 40, height: 40, cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
          >
            ➤
          </button>
        </div>
