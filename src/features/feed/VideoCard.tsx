            ...
            {showMenu && (
              <div style={{ position: 'absolute', right: 0, top: 24, background: '#FFFFFF', borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.15)', border: '1px solid rgba(0,0,0,0.08)', zIndex: 50, minWidth: 160, overflow: 'hidden' }}>
                {currentUserId !== post.userId ? (
                  <>
                    <button onClick={async () => { setShowMenu(false); if (!currentUserId) return; await addDoc(collection(firebaseDb(), 'reports'), { postId: post.id, reportedBy: currentUserId, reason: 'inappropriate', createdAt: serverTimestamp() }); alert('Reported. Thank you.'); }} style={{ width: '100%', padding: '12px 16px', background: 'none', border: 'none', textAlign: 'left', fontSize: 14, cursor: 'pointer', color: '#1A1A1A', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>🚩 Report post</button>
                    <button onClick={async () => { setShowMenu(false); if (!currentUserId) return; await updateDoc(doc(firebaseDb(), 'users', currentUserId), { blockedUsers: arrayUnion(post.userId) }); alert(`@${post.username} has been blocked.`); }} style={{ width: '100%', padding: '12px 16px', background: 'none', border: 'none', textAlign: 'left', fontSize: 14, cursor: 'pointer', color: '#EF4444' }}>🚫 Block @{post.username}</button>
                  </>
                ) : (
