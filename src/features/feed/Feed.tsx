...
// Stories matching the design - Ethiopian names
const STORIES = [
  { name: 'Selam', color: '#5A9A6F' },
  { name: 'Yared', color: '#3D6B4F' },
  { name: 'Hana', color: '#8B6B4F' },
  { name: 'Elias', color: '#4F6B8B' },
];
...
export function Feed({ currentUserId, followedIds, onComment, onShare, onViewProfile, onFollow }: FeedProps) {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: '#F8F7F4' }}>
      ...
      <div style={{ background: '#FFFFFF', padding: '14px 18px 0', position: 'sticky', top: 0, zIndex: 20, borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <span style={{ position: 'absolute', top: -2, right: -2, width: 7, height: 7, background: '#EF4444', borderRadius: '50%', border: '1.5px solid white' }} />
          </div>
        </div>
      ...
      {loading
        ? [0,1].map(i => (
            <div key={i} style={{ background: '#fff', borderRadius: 16, margin: '10px 14px', overflow: 'hidden', height: 320 }}>
              <VideoCardSkeleton />
            </div>
          ))
        : posts.length === 0
