    ...
    <div style={{ height: '100%', background: '#F5F5F7', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: 'rgba(245,245,247,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {unreadCount > 0 && (
            <span style={{ background: '#5B4EE8', color: 'white', borderRadius: 999, padding: '2px 8px', fontSize: 12, fontWeight: 700 }}>
              {unreadCount}
            </span>
          )}
        </div>
        {unreadCount > 0 && (
          <button onClick={markAllRead} style={{ background: 'none', border: 'none', color: '#5B4EE8', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
            Mark all read
          </button>
        )}
      </div>
        ...
        {FILTERS.map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)}
            style={{
              ...
              color: filter === f.id ? '#5B4EE8' : '#6B7280',
            ...
            <div key={group}>
              ...
              {groupedItems.map(n => (
                <button key={n.id} onClick={() => markOneRead(n.id)}
                  ...
                  {!n.read && (
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#5B4EE8', flexShrink: 0 }} />
                  )}
                  {n.videoUrl && (
