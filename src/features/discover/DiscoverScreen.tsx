...
import { collection, getDocs, query, where, orderBy, limit, doc, updateDoc, arrayUnion, arrayRemove, getDoc } from 'firebase/firestore';
...
export function DiscoverScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const { getDocs: gd, collection: col, query: q, orderBy: ob, limit: lim, where: wh } = await import('firebase/firestore');
  ...
  const toggleFollow = async (target: UserResult) => {
    ...
    try {
      ...
      const theirRef = doc(firebaseDb(), 'users', target.id);
      if (isFollowing) {
        await Promise.all([
          updateDoc(myRef, { following: arrayRemove(target.id) }),
          updateDoc(theirRef, { followers: arrayRemove(user.id) }),
        ]);
        setFollowedIds(prev => { const n = new Set(prev); n.delete(target.id); return n; });
        showToast('Unfollowed', 'info');
      } else {
        await Promise.all([
          updateDoc(myRef, { following: arrayUnion(target.id) }),
          updateDoc(theirRef, { followers: arrayUnion(user.id) }),
        ]);
        setFollowedIds(prev => new Set([...prev, target.id]));
        await createNotification({
          userId: target.id, fromUserId: user.id, fromUsername: user.username,
