...
export function AuthScreen() {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'#F5F5F7', padding:24, position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ position:'relative', zIndex:1, width:'100%', maxWidth:360 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <button type="submit" disabled={busy} className="gradient-brand" style={{ border:'none', color:'#0D0D12', borderRadius:16, padding:'14px', fontSize:15, fontWeight:700, cursor:busy?'not-allowed':'pointer', opacity:busy?0.6:1, marginTop:4 }}>
  ...
  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6, marginTop:10 }}>
    ...
    <button type="button" onClick={handleResendVerification} disabled={busy} style={{ background:'none', border:'none', color:'rgba(255,255,255,0.35)', fontSize:12, cursor:busy?'not-allowed':'pointer' }}>
      Resend verification email
        ...
        <button onClick={handleGoogle} disabled={busy} style={{ width:'100%', background:'rgba(255,255,255,0.07)', border:'1px solid rgba(0,0,0,0.08)', color:'#0D0D12', borderRadius:16, padding:'14px', fontSize:15, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
          🌐 Continue with Google
        </button>

        <p style={{ textAlign:'center', marginTop:24, fontSize:14, color:'rgba(255,255,255,0.5)' }}>
          {mode==='login' ? "Don't have an account? " : 'Already have one? '}
          <button type="button" onClick={()=>setMode(mode==='login'?'signup':'login')} style={{ background:'none', border:'none', color:'#6B4EFF', fontWeight:700, cursor:'pointer', fontSize:14 }}>
