...
function ChatView({ conv, onBack }: { conv: Conversation; onBack: () => void }) {
  ...
  const otherColor = getField(conv.participantAvatarColors, otherId, '#5B4EE8');
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: '#0A0A0F', display: 'flex', flexDirection: 'column' }}>
          ...
          return (
            <div key={m.id} style={{
              ...
              <div style={{
                ...
                borderRadius,
                background: isMine
                  ? 'linear-gradient(135deg,#5B4EE8,#9B4FFF)'
      ...
      <div style={{
        ...
        <button onClick={() => send()} disabled={!text.trim() || sending} style={{
          ...
          background: text.trim() ? 'linear-gradient(135deg,#5B4EE8,#9B4FFF)' : 'rgba(255,255,255,0.06)',
          border: '1px solid rgba(255,255,255,0.07)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
          ...
          <button onClick={search} style={{
            background: 'linear-gradient(135deg,#5B4EE8,#9B4FFF)',
  ...
  return (
    <div style={{ display: 'flex', gap: 14, padding: '4px 16px 16px', overflowX: 'auto', scrollbarWidth: 'none' }}>
      {conversations.slice(0, 6).map(conv => {
        ...
        const color = getField(conv.participantAvatarColors, otherId, '#5B4EE8');
      ...
      <div style={{ padding: '18px 16px 10px', flexShrink: 0, background: 'rgba(245,245,247,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <button onClick={() => setShowNew(true)} style={{
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#5B4EE8" strokeWidth="2.5" strokeLinecap="round">
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0 80px' }}>
        ...
        {!loading && conversations.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '65%', gap: 14, padding: 32 }}>
            ...
            <button onClick={() => setShowNew(true)} style={{ background: '#5B4EE8', border: 'none', color: '#fff', borderRadius: 14, padding: '12px 28px', fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>New message</button>
          </div>
        )}
        ...
        {filtered.map(conv => {
          ...
          return (
            <button key={conv.id} onClick={() => setActiveConv(conv)} style={{
              ...
              {hasUnread && (
                <div style={{ width: 20, height: 20, borderRadius: 999, flexShrink: 0, background: '#5B4EE8', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 10, fontWeight: 800 }}>
