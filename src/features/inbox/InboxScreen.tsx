...
function OnlineRing({ color, size = 52 }: { color: string; size?: number }) {
  return (
    <div style={{
      ...
      background: `linear-gradient(135deg, ${color}, #5A9A6F)`,
    }} />
  );
}
...
function ChatView({ conv, onBack }: { conv: Conversation; onBack: () => void }) {
  ...
  const otherColor = getField(conv.participantAvatarColors, otherId, '#3D6B4F');
  ...
  // ✅ FIXED: Uses API instead of direct Firestore write
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: '#0A0A0F', display: 'flex', flexDirection: 'column' }}>
          ...
          return (
            <div key={m.id} style={{
              ...
              <div style={{
                ...
                borderRadius,
                background: isMine
                  ? 'linear-gradient(135deg,#3D6B4F,#5A9A6F)'
      ...
      <div style={{
        ...
        <button onClick={() => send()} disabled={!text.trim() || sending} style={{
          ...
          background: text.trim() ? 'linear-gradient(135deg,#3D6B4F,#5A9A6F)' : 'rgba(255,255,255,0.06)',
          border: '1px solid rgba(255,255,255,0.07)',
...
function NewMessageModal({ onClose, onStart }: { onClose: () => void; onStart: (c: Conversation) => void }) {
  ...
  const startChat = async (other: UserResult) => {
    ...
    try {
      ...
      const convSnap = await getDoc(doc(firebaseDb(), 'conversations', data.conversationId));
      onStart({ id: data.conversationId, ...convSnap.data() } as Conversation);
    } catch {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'flex-end' }}>
      <div style={{
        ...
        {/* drag handle */}
        <div style={{ width: 36, height: 4, background: 'rgba(0,0,0,0.12)', borderRadius: 99, margin: '14px auto 20px' }} />

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <div style={{ fontWeight: 900, fontSize: 20, color: '#1A1A1A', letterSpacing: '-0.5px' }}>New message</div>
            <div style={{ fontSize: 12, color: '#9CA3AF', marginTop: 2 }}>find someone to chat with</div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
          <div style={{
            ...
            <input
              ...
              style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: '#1A1A1A', fontSize: 14 }}
              autoFocus
            />
          </div>
        ...
        <div style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {results.map(u => (
            <button key={u.id} onClick={() => startChat(u)} style={{
              ...
              cursor: 'pointer', color: '#1A1A1A', textAlign: 'left',
            }}>
...
export function InboxScreen() {
  ...
  return (
    <div style={{ height: '100%', background: '#F8F7F4', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

      {/* Header */}
      <div style={{ padding: '18px 16px 10px', flexShrink: 0, background: 'rgba(248,247,244,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: '#1A1A1A', margin: 0 }}>Messages</h1>
          <button onClick={() => setShowNew(true)} style={{
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3D6B4F" strokeWidth="2.5" strokeLinecap="round">
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0 80px' }}>
        ...
        {!loading && conversations.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '65%', gap: 14, padding: 32 }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 18, color: '#1A1A1A' }}>No messages yet</div>
            ...
            <button onClick={() => setShowNew(true)} style={{ background: '#3D6B4F', border: 'none', color: '#fff', borderRadius: 14, padding: '12px 28px', fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>New message</button>
          </div>
        )}
            ...
            <button key={conv.id} onClick={() => setActiveConv(conv)} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                  <span style={{ fontWeight: hasUnread ? 800 : 600, fontSize: 14, color: '#1A1A1A' }}>{otherName}</span>
                  <span style={{ fontSize: 11, color: '#9CA3AF', flexShrink: 0, marginLeft: 8 }}>{timeLabel}</span>
                </div>
                <div style={{ fontSize: 13, color: hasUnread ? '#1A1A1A' : '#9CA3AF', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: hasUnread ? 600 : 400 }}>
                  {conv.lastMessage || 'Say hello 👋'}
                </div>
              </div>

              {hasUnread && (
                <div style={{ width: 20, height: 20, borderRadius: 999, flexShrink: 0, background: '#3D6B4F', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 10, fontWeight: 800 }}>
