  ...
  const submit = async () => {
    ...
    try {
      ...
      const id = await publishVideo({
        userId: user.id, username: user.username, avatar: user.avatar, avatarColor: user.avatarColor,
        avatarUrl: user.avatarUrl, verified: user.verified, description, hashtags,
        mediaUrl: uploaded.url, mediaType: file.type.startsWith('image/') ? 'image' : 'video',
        song: 'Original sound', moderationStatus: verdict.safe ? 'approved' : 'flagged',
        moderationFlags: verdict.flags, trendingScore: 1,
      });
          ...
          <div style={{ display:'flex', gap:12 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="What's on your mind?" rows={4}
                style={{ width:'100%', background:'transparent', border:'none', color:'#0D0D12', fontSize:15, resize:'none', outline:'none', lineHeight:1.5 }} maxLength={500} />
            </div>
          </div>
        </div>
