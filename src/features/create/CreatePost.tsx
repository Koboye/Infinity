  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: '#FFFFFF', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16, background: '#F8F7F4' }}>
        ...
        {preview && (
          <div style={{ position: 'relative', marginBottom: 12, borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.06)', background: '#F8F7F4' }}>
        ...
        {hashtags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {hashtags.map(t => <span key={t} style={{ background: 'rgba(61,107,79,0.1)', color: '#3D6B4F', borderRadius: 999, padding: '4px 12px', fontSize: 13, fontWeight: 700 }}>{t}</span>)}
          </div>
        )}
