  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: '#FFFFFF', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: 'rgba(245,245,247,0.97)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,0,0,0.06)', position: 'sticky', top: 0 }}>
        ...
        <button onClick={submit} disabled={submitting || !file} style={{ background: submitting || !file ? 'rgba(0,0,0,0.08)' : 'linear-gradient(135deg,#5B4EE8,#9D7BFF)', border: 'none', color: submitting || !file ? '#9CA3AF' : 'white', borderRadius: 999, padding: '6px 16px', fontWeight: 700, cursor: 'pointer' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16, background: '#F5F5F7' }}>
        ...
        {hashtags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {hashtags.map(t => <span key={t} style={{ background: 'rgba(107,78,255,0.1)', color: '#5B4EE8', borderRadius: 999, padding: '4px 12px', fontSize: 13, fontWeight: 700 }}>{t}</span>)}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
          ...
          <button onClick={smartCaption} disabled={aiLoading} style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', color: '#5B4EE8', borderRadius: 14, padding: '12px', cursor: 'pointer', fontSize: 13, fontWeight: 600, opacity: aiLoading ? 0.5 : 1 }}>✨ {aiLoading ? 'Generating…' : 'Smart Caption'}</button>
        ...
        {submitting && progress > 0 && (
          <div style={{ padding: 12, borderRadius: 14, background: '#FFFFFF', marginBottom: 12, border: '1px solid rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ height: 6, background: 'rgba(0,0,0,0.06)', borderRadius: 3 }}>
              <div style={{ height: '100%', borderRadius: 3, width: `${progress}%`, transition: 'width 0.3s', background: 'linear-gradient(135deg,#5B4EE8,#9D7BFF)' }} />
            </div>
          </div>
        )}
      </div>
