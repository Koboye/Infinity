...
function FollowListModal({ title, userIds, onClose }: { title: string; userIds: string[]; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'flex-end' }}>
      <div style={{ width: '100%', background: '#1C1C24', borderRadius: '24px 24px 0 0', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: 12 }}>
          {loading && <div style={{ textAlign: 'center', padding: 32, color: 'rgba(255,255,255,0.4)' }}>Loading…</div>}
          {!loading && users.length === 0 && (
            <div style={{ textAlign: 'center', padding: 32, color: 'rgba(255,255,255,0.4)' }}>No users yet</div>
          )}
          {users.map(u => (
    ...
    <div style={{ height: '100%', overflowY: 'auto', background: '#0B0B0F' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={shareProfile} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', color: 'white', borderRadius: 12, padding: '8px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>🔗 Share</button>
      ...
      <div style={{ padding: '24px 16px', textAlign: 'center' }}>
        ...
        {user.link && (
          <a href={user.link} target="_blank" rel="noopener noreferrer" style={{ color: '#FF2156', fontSize: 13, marginTop: 6, display: 'block' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-around', background: 'rgba(255,255,255,0.04)', borderRadius: 16, padding: 16, margin: '16px 0', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          ].map(({ label, val, onClick }, i, arr) => (
            <button key={label} onClick={onClick}
              style={{ textAlign: 'center', flex: 1, borderRight: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none', background: 'none', border: 'none', cursor: onClick ? 'pointer' : 'default', color: 'white', padding: 0 }}>
              <div style={{ fontSize: 18, fontWeight: 900 }}>{formatCount(val)}</div>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          <div style={{ background: 'rgba(255,33,86,0.08)', border: '1px solid rgba(255,33,86,0.15)', borderRadius: 12, padding: '10px 16px', textAlign: 'center', flex: 1 }}>
          ...
          <div style={{ background: 'rgba(157,78,221,0.08)', border: '1px solid rgba(157,78,221,0.15)', borderRadius: 12, padding: '10px 16px', textAlign: 'center', flex: 1 }}>
            <div style={{ fontSize: 16, fontWeight: 900, color: '#9D4EDD' }}>{formatCount(totalLikes)}</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>❤️ Likes</div>
          </div>
          <div style={{ background: 'rgba(10,132,255,0.08)', border: '1px solid rgba(10,132,255,0.15)', borderRadius: 12, padding: '10px 16px', textAlign: 'center', flex: 1 }}>
      ...
      <div style={{ display: 'flex', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        {([['posts', '🎬'], ['liked', '❤️'], ['saved', '🔖']] as const).map(([t, icon]) => (
          <button key={t} onClick={() => setTab(t)}
            style={{ flex: 1, padding: '14px', background: 'none', border: 'none', borderBottom: tab === t ? '2px solid #FF2156' : '2px solid transparent', color: tab === t ? 'white' : 'rgba(255,255,255,0.4)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
      ...
      {tab === 'posts' && (
        posts.length === 0
          ? <div style={{ textAlign: 'center', padding: '48px 24px', color: 'rgba(255,255,255,0.4)' }}>
              ...
              <div style={{ fontSize: 14 }}>No posts yet. Tap + to create your first!</div>
            </div>
      ...
      {tab === 'liked' && (
        <div style={{ textAlign: 'center', padding: '48px 24px', color: 'rgba(255,255,255,0.4)' }}>
          ...
          <div style={{ fontSize: 14 }}>Videos you liked appear here</div>
        </div>
      )}
      ...
      {tab === 'saved' && (
        <div style={{ textAlign: 'center', padding: '48px 24px', color: 'rgba(255,255,255,0.4)' }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>🔖</div>
          <div style={{ fontSize: 14 }}>Videos you saved appear here</div>
        </div>
      )}
